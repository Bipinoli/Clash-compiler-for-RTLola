...
-- 
-- output a := x + 1
-- output b @1kHz := a.hold(or: 0) + e.hold(or: 1)
-- output c @x := b.hold(or: 0) + f.hold(or: 1) 
-- output d @0.5kHz := c.hold(or: 3) + 1
-- output e := d.hold(or: 0) + a
-- output f @0.25kHz := b.hold(or: 10)  
-- 
...
nullPacings = Pacings False False False False False False 
...
hlc inputs = out
    where 
        ...
        newEvent = hasInput0 .||. p1 .||. p3 .||. p5

        event = bundle (inputs, pacings)

        pacings = Pacings <$> p0 <*> p1 <*> p2 <*> p3 <*> p4 <*> p5
...
llc event = bundle (bundle (toPop, outputs), debugSignals)
    where 
        ...
        out1Data1 = getLatestValueFromNonVec <$> out4 <*> out1Data1Dflt
        out1Data1Dflt = pure (1)

        -- Evaluation of output 2: level 0
        out2 = outputStream2 enOut2 tOut2 out2Data0 out2Data1 
        out2Data0 = getLatestValueFromNonVec <$> out1 <*> out2Data0Dflt
        out2Data0Dflt = pure (0)
        out2Data1 = getLatestValueFromNonVec <$> out5 <*> out2Data1Dflt
        ...
        out3Data0Dflt = pure (3)

        -- Evaluation of output 4: level 2
        out4 = outputStream4 enOut4 ((.output4) <$> curTagsLevel2) out4Data0 out4Data1 
        out4Data0 = getMatchingTagFromNonVec <$> out0 <*> ((.output0) <$> curTagsLevel2) <*> (pure (0))
        out4Data1 = getLatestValueFromNonVec <$> out3 <*> out4Data1Dflt
        out4Data1Dflt = pure (0)

        -- Evaluation of output 5: level 4
        out5 = outputStream5 enOut5 ((.output5) <$> curTagsLevel4) out5Data0 
        out5Data0 = getLatestValueFromNonVec <$> out1 <*> out5Data0Dflt
        out5Data0Dflt = pure (10)
...
outputStream1 en tag out0 out4 = result
    where
        result = register (invalidTag, 0) (mux en nextValWithTag result)
        nextValWithTag = bundle (tag, nextVal)
        nextVal = out0 + out4
...
outputStream2 en tag out1 out5 = result
    where
        result = register (invalidTag, 0) (mux en nextValWithTag result)
        nextValWithTag = bundle (tag, nextVal)
        nextVal = out1 + out5
...
outputStream4 :: HiddenClockResetEnable dom => Signal dom Bool -> Signal dom Tag -> Signal dom Int -> Signal dom Int -> Signal dom (Tag, Int)
outputStream4 en tag out0 out3 = result
    where
        result = register (invalidTag, 0) (mux en nextValWithTag result)
        nextValWithTag = bundle (tag, nextVal)
        nextVal = out3 + out0
...
outputStream5 en tag out1 = result
    where
        result = register (invalidTag, 0) (mux en nextValWithTag result)
        nextValWithTag = bundle (tag, nextVal)
        nextVal = out1
