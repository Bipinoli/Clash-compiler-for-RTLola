...
-- -----------------------------------------------------------------------------------------------------------------------------------------
--             | a,b,sw(x,f) |             |             | a,b,sw(x,f) |             |             | a,b,sw(x,f) |             |            
-- -----------------------------------------------------------------------------------------------------------------------------------------
--             |             | c,f         |             |             | c,f         |             |             | c,f         |            
-- -----------------------------------------------------------------------------------------------------------------------------------------
--             |             |             | d           |             |             | d           |             |             | d          
...
llc event = bundle (bundle (toPop, outputs), debugSignals)
    where 
        ...
        enSw0 = delay False (delay False (slide0 .||. input0HasData))
        ...
        enSw1 = delay False (delay False (delay False (delay False (delay False (slide1 .||. p3)))))
        ...
        out2Data2 = getMatchingTagFromNonVec <$> out0 <*> out2Level2TagOut0 <*> (pure 0)
        out2Data3 = getMatchingTagFromNonVec <$> out1 <*> out2Level2TagOut1 <*> (pure 0)
        ...
        out3Data0 = getMatchingTagFromNonVec <$> out0 <*> out3Level3TagOut0 <*> (pure 0)
        out3Data1 = getMatchingTagFromNonVec <$> out2 <*> out3Level3TagOut2 <*> (pure 0)
        ...
        out6Data2 = getMatchingTagFromNonVec <$> out5 <*> out6Level4TagOut5 <*> (pure 0)
...
outputStream2 :: HiddenClockResetEnable dom => Signal dom Bool -> Signal dom Tag -> Signal dom (Tag, Int) -> Signal dom (Tag, Int) -> Signal dom (Tag, Int) -> Signal dom (Tag, Int) -> Signal dom (Tag, Int)
...
outputStream5 :: HiddenClockResetEnable dom => Signal dom Bool -> Signal dom Tag -> Signal dom (Tag, (Vec 4 Int)) -> Signal dom (Tag, Int)
