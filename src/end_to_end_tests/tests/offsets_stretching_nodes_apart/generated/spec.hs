...
llc event = bundle (bundle (toPop, outputs), debugSignals)
    where 
        ...
        out6Data0 = getMatchingTag 
            ...
            <*> (pure (0))
        out6Data1 = getOffsetFromNonVec 
            <$> out11 
            <*> ((.output11) <$> curTagsLevel5) 
            <*> (pure 1) 
            <*> out6Data1Dflt
        ...
        out8 = outputStream8 enOut8 
            ((.output8) <$> curTagsLevel8) 
            out8Data0 
            out8Data1 
        out8Data0 = getMatchingTagFromNonVec 
            <$> out6 
            <*> ((.output6) <$> curTagsLevel8) 
            <*> (pure (0))
        out8Data1 = getMatchingTagFromNonVec 
            <$> out10 
            <*> ((.output10) <$> curTagsLevel8) 
            <*> (pure (0))
        ...
        out11 = outputStream11 enOut11 
            ((.output11) <$> curTagsLevel9) 
            out11Data0 
        out11Data0 = getMatchingTagFromNonVec 
            <$> out8 
            <*> ((.output8) <$> curTagsLevel9) 
            <*> (pure (0))
