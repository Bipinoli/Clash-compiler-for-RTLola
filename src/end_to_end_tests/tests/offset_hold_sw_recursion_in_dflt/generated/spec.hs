...
llc event = bundle (bundle (toPop, outputs), debugSignals)
    where 
        ...
        curTags = Tags <$> tIn0 <*> tOut0 <*> tOut1 <*> tOut2 <*> tSw0 <*> tSw1
        ...
        out0Data1 = getOffsetFromNonVec <$> out1 <*> ((.output1) <$> curTagsLevel1) <*> (pure 1) <*> out0Data1Dflt
        out0Data1Dflt = getOffset <$> input0Win <*> ((.input0) <$> curTagsLevel1) <*> (pure (1)) <*> (pure (0))

        -- Evaluation of output 1: level 2
        out1 = outputStream1 enOut1 ((.output1) <$> curTagsLevel2) out1Data0 out1Data1 
        out1Data0 = getLatestValueFromNonVec <$> out2 <*> out1Data0Dflt
        out1Data0Dflt = getOffset <$> out0 <*> ((.output0) <$> curTagsLevel2) <*> (pure (1)) <*> (pure (0))
        ...
        sw1Data = getMatchingTag <$> out0 <*> ((.output0) <$> curTagsLevel3) <*> (pure 0)
...
slidingWindow0 newData slide tag inpt = window
    where
        ...
        nextWindow :: Vec 101 Int -> Bool -> Bool -> Int -> Vec 101 Int
