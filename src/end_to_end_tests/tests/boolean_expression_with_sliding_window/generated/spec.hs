...
llc event = bundle (bundle (toPop, outputs), debugSignals)
    where 
        ...
        curTags = Tags <$> tIn0 <*> tOut0 <*> tOut1 <*> tOut2 <*> tOut3 <*> tOut4 <*> tOut5 <*> tSw0
        ...
        out0Data0 = getLatestValue <$> input0Win <*> out0Data0Dflt
        ...
        out3Data2 = getOffset <$> out1 <*> ((.output1) <$> curTagsLevel3) <*> (pure 1) <*> out3Data2Dflt
        out3Data2Dflt = pure (False)
        out3Data3 = getMatchingTag <$> out2 <*> ((.output2) <$> curTagsLevel3) <*> (pure (False))
        ...
        out5Data0 = getLatestValue <$> input0Win <*> out5Data0Dflt
        ...
        output1Data = getMatchingTag <$> out1 <*> ((.output1) <$> curTagsLevel6) <*> (pure False)
        output2 = ValidBool <$> output2Data <*> output2Aktv
        output2Data = getMatchingTag <$> out2 <*> ((.output2) <$> curTagsLevel6) <*> (pure False)
        output3 = ValidBool <$> output3Data <*> output3Aktv
        output3Data = getMatchingTag <$> out3 <*> ((.output3) <$> curTagsLevel6) <*> (pure False)
...
outputStream0 :: HiddenClockResetEnable dom => Signal dom PacingOut0 -> Signal dom Tag -> Signal dom Int -> Signal dom (Vec 5 (Tag, Int))
...
outputStream1 :: HiddenClockResetEnable dom => Signal dom PacingOut1 -> Signal dom Tag -> Signal dom Int -> Signal dom Int -> Signal dom (Vec 4 (Tag, Bool))
...
outputStream2 :: HiddenClockResetEnable dom => Signal dom PacingOut2 -> Signal dom Tag -> Signal dom Int -> Signal dom Int -> Signal dom (Vec 4 (Tag, Bool))
...
outputStream3 :: HiddenClockResetEnable dom => Signal dom PacingOut3 -> Signal dom Tag -> Signal dom Bool -> Signal dom Bool -> Signal dom Bool -> Signal dom Bool -> Signal dom (Vec 3 (Tag, Bool))
...
outputStream4 :: HiddenClockResetEnable dom => Signal dom PacingOut4 -> Signal dom Tag -> Signal dom (Vec 21 Int) -> Signal dom (Tag, Int)
...
slidingWindow0 newData slide tag inpt = window
    where
        ...
        nextWindow :: Vec 21 Int -> Bool -> Bool -> Bool -> Vec 21 Int
