...
-- input x: Int
-- output a: Int := b.offset(by: -1).defaults(to: x)
-- output b: Int := a.hold().defaults(to: 2 + x)
...
nullPacings = Pacings nullPacingIn0 nullPacingOut0 nullPacingOut1 
...
hlc inputs = out
    where 
        ...
        pacings = Pacings <$> pIn0 <*> pOut0 <*> pOut1
...
llc event = bundle (bundle (toPop, outputs), debugSignals)
    where 
        ...
        out0Data0 = getOffsetFromNonVec <$> out1 <*> ((.output1) <$> curTagsLevel1) <*> (pure 1) <*> out0Data0Dflt
        (_, out0Data0Dflt) = unbundle input0Win
        ...
        out1Data0DfltData0 = pure (2)
        (_, out1Data0DfltData1) = unbundle input0Win
...
outputStream0 en tag out1_ = result
    where
