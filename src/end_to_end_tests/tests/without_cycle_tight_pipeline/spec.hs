...
instance Pacing PacingOut2 where getPacing (PacingOut2 x y) = getPacing x && getPacing y
instance Pacing PacingOut3 where getPacing (PacingOut3 x y z) = getPacing x && getPacing y && getPacing z
...
llc event = bundle (bundle (toPop, outputs), debugSignals)
    where 
        ...
        output1Aktv = delayFor d6 False (getPacing <$> pOut0)
