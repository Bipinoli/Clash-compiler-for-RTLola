/* AUTOMATICALLY GENERATED VERILOG-2001 SOURCE CODE.
** GENERATED BY CLASH 1.8.2. DO NOT MODIFY.
*/
`default_nettype none
`timescale 100fs/100fs
module topEntity
    ( // Inputs
      input wire  clk // clock
    , input wire  rst // reset
    , input wire  en // enable
    , input wire signed [63:0] inputs_0_0
    , input wire  inputs_0_1
    , input wire signed [63:0] inputs_1_0
    , input wire  inputs_1_1

      // Outputs
    , output wire signed [63:0] result_0_0_0
    , output wire  result_0_0_1
    , output wire signed [63:0] result_0_1_0
    , output wire  result_0_1_1
    , output wire signed [63:0] result_0_2_0
    , output wire  result_0_2_1
    , output wire signed [63:0] result_0_3_0
    , output wire  result_0_3_1
    , output wire  result_0_4_0
    , output wire  result_0_4_1
    , output wire signed [63:0] result_0_5_0
    , output wire  result_0_5_1
    , output wire  result_1_0
    , output wire  result_1_1
    , output wire  result_1_2
    , output wire  result_1_3
    , output wire  result_1_4
    , output wire  result_1_5_0
    , output wire  result_1_5_1
    , output wire  result_1_5_2
    , output wire  result_1_5_3
    , output wire  result_1_5_4
    , output wire  result_1_5_5
    , output wire  result_1_5_6
    , output wire  result_1_5_7
    );
  wire signed [63:0] result_2;
  wire signed [63:0] x;
  reg signed [63:0] result_3 = (64'sd0);
  wire [144:0] result_4;
  // spec.hs:(285,1)-(287,31)
  wire  pIn0;
  // spec.hs:(285,1)-(287,31)
  wire  pIn1;
  // spec.hs:(285,1)-(287,31)
  wire  timer0Over;
  wire [336:0] result_5;
  // spec.hs:116:34-42
  wire  x1;
  // spec.hs:116:34-42
  wire  x0;
  // spec.hs:115:34-42
  wire  x1_0;
  // spec.hs:115:34-42
  wire  x0_0;
  // spec.hs:114:34-42
  wire  x1_1;
  // spec.hs:114:34-42
  wire  x0_1;
  // spec.hs:113:34-42
  wire  x1_2;
  // spec.hs:113:34-42
  wire  x0_2;
  // spec.hs:112:34-42
  wire  x1_3;
  // spec.hs:112:34-42
  wire  x0_3;
  wire [7:0] c$app_arg;
  wire  result_6;
  wire [7:0] c$app_arg_0;
  wire  result_7;
  wire [7:0] c$app_arg_1;
  wire  result_8;
  // spec.hs:349:1-63
  wire [147:0] c$ws_app_arg;
  // spec.hs:349:1-63
  wire [3:0] c$ws_app_arg_0;
  wire [71:0] c$case_scrut;
  wire [1:0] c$case_scrut_0;
  wire signed [63:0] result_9;
  // spec.hs:349:1-63
  wire [0:0] i;
  // spec.hs:349:1-63
  wire [5:0] ws;
  wire [7:0] c$app_arg_2;
  wire  result_10;
  wire [143:0] result_11;
  // spec.hs:114:34-42
  wire  x0_4;
  // spec.hs:114:34-42
  wire  x1_4;
  // spec.hs:(700,1)-(705,36)
  reg [143:0] result_12 = {{8'd13,   64'sd0},   {8'd13,   64'sd0}};
  // spec.hs:(700,1)-(705,36)
  wire [143:0] t;
  wire signed [63:0] x_0;
  wire signed [63:0] y;
  // spec.hs:341:1-65
  wire [224:0] c$ws_app_arg_1;
  // spec.hs:341:1-65
  wire [8:0] c$ws_app_arg_2;
  wire [71:0] c$case_scrut_1;
  wire [2:0] c$case_scrut_2;
  wire signed [63:0] result_13;
  wire [7:0] result_14;
  wire [7:0] c$app_arg_3;
  // spec.hs:341:1-65
  wire [1:0] i_0;
  // spec.hs:341:1-65
  wire [11:0] ws_0;
  // spec.hs:349:1-63
  wire [224:0] c$ws_app_arg_3;
  // spec.hs:349:1-63
  wire [8:0] c$ws_app_arg_4;
  wire [71:0] c$case_scrut_3;
  wire [2:0] c$case_scrut_4;
  wire signed [63:0] result_15;
  // spec.hs:349:1-63
  wire [1:0] i_1;
  // spec.hs:349:1-63
  wire [11:0] ws_1;
  wire [7:0] c$app_arg_4;
  wire  result_16;
  // spec.hs:349:1-63
  wire [147:0] c$ws_app_arg_5;
  // spec.hs:349:1-63
  wire [3:0] c$ws_app_arg_6;
  wire [71:0] c$case_scrut_5;
  wire [1:0] c$case_scrut_6;
  wire signed [63:0] result_17;
  // spec.hs:349:1-63
  wire [0:0] i_2;
  // spec.hs:349:1-63
  wire [5:0] ws_2;
  wire [7:0] c$app_arg_5;
  wire  result_18;
  // spec.hs:349:1-63
  wire [224:0] c$ws_app_arg_7;
  // spec.hs:349:1-63
  wire [8:0] c$ws_app_arg_8;
  wire [71:0] c$case_scrut_7;
  wire [2:0] c$case_scrut_8;
  wire signed [63:0] result_19;
  // spec.hs:349:1-63
  wire [1:0] i_3;
  // spec.hs:349:1-63
  wire [11:0] ws_3;
  wire [215:0] result_20;
  // spec.hs:113:34-42
  wire  x0_5;
  // spec.hs:113:34-42
  wire  x1_5;
  // spec.hs:(684,1)-(690,36)
  reg [215:0] result_21 = {{8'd13,   64'sd0},   {8'd13,   64'sd0},   {8'd13,   64'sd0}};
  // spec.hs:(684,1)-(690,36)
  wire [215:0] t_0;
  wire signed [63:0] x_1;
  wire signed [63:0] y_0;
  wire signed [63:0] x_2;
  wire signed [63:0] y_1;
  // spec.hs:(393,1)-(395,59)
  wire signed [63:0] c$out1_case_alt;
  wire [71:0] result_22;
  // spec.hs:115:34-42
  wire  x0_6;
  // spec.hs:115:34-42
  wire  x1_6;
  // spec.hs:(715,1)-(721,28)
  reg [71:0] result_23 = {8'd13,   64'sd0};
  // spec.hs:(715,1)-(721,28)
  wire [71:0] t_1;
  wire signed [63:0] x_3;
  wire signed [63:0] y_2;
  wire signed [63:0] x_4;
  wire signed [63:0] y_3;
  // spec.hs:(393,1)-(395,59)
  wire signed [63:0] c$out3_case_alt;
  // spec.hs:371:1-46
  wire signed [63:0] dta;
  // spec.hs:371:1-46
  wire [7:0] tag;
  // spec.hs:(744,1)-(748,28)
  wire signed [63:0] c$t_app_arg;
  wire [71:0] result_24;
  // spec.hs:(744,1)-(748,28)
  reg [71:0] result_25 = {8'd13,   64'sd0};
  // spec.hs:(744,1)-(748,28)
  wire  b;
  // spec.hs:(744,1)-(748,28)
  wire [71:0] t_2;
  // spec.hs:(767,1)-(772,37)
  wire signed [63:0] x_5;
  // spec.hs:(767,1)-(772,37)
  wire [703:0] win;
  // spec.hs:(767,1)-(772,37)
  reg [711:0] window = {8'd13,   {64'sd0,   64'sd0,   64'sd0,   64'sd0,   64'sd0,   64'sd0,   64'sd0,
           64'sd0,   64'sd0,   64'sd0,   64'sd0}};
  wire [711:0] result_26;
  // spec.hs:(767,1)-(772,37)
  wire [711:0] t_3;
  wire [703:0] result_27;
  // spec.hs:(767,1)-(772,37)
  wire [703:0] c$t_case_alt;
  // spec.hs:(767,1)-(772,37)
  wire [703:0] c$t_case_alt_0;
  // spec.hs:(767,1)-(772,37)
  wire [703:0] lastBucketUpdated;
  // spec.hs:(767,1)-(772,37)
  wire  b_0;
  // spec.hs:(767,1)-(772,37)
  wire  c$b_case_alt;
  // spec.hs:116:34-42
  wire  x1_7;
  // spec.hs:116:34-42
  wire  x0_7;
  wire [8:0] result_28;
  // spec.hs:116:34-42
  wire  x0_8;
  // spec.hs:116:34-42
  wire  x1_8;
  // spec.hs:(730,1)-(735,29)
  reg [8:0] result_29 = {8'd13,   1'b0};
  // spec.hs:(730,1)-(735,29)
  wire [8:0] t_4;
  // spec.hs:(393,1)-(395,59)
  wire signed [63:0] c$out4_case_alt;
  // spec.hs:362:1-53
  wire signed [63:0] dta_0;
  // spec.hs:362:1-53
  wire [7:0] tag_0;
  // spec.hs:362:1-53
  wire [7:0] tagToMatch;
  wire [359:0] c$app_arg_6;
  wire [71:0] result_30;
  wire [5:0] c$app_arg_7;
  wire  result_31;
  // spec.hs:(393,1)-(395,59)
  wire  slides;
  wire [11:0] c$app_arg_8;
  wire [1:0] result_32;
  wire [431:0] c$app_arg_9;
  wire [71:0] result_33;
  wire [7:0] result_34;
  wire [7:0] c$app_arg_10;
  // spec.hs:356:1-55
  wire [7:0] tag_1;
  // spec.hs:349:1-63
  wire [147:0] c$ws_app_arg_9;
  // spec.hs:349:1-63
  wire [3:0] c$ws_app_arg_10;
  wire [71:0] c$case_scrut_9;
  wire [1:0] c$case_scrut_10;
  wire signed [63:0] result_35;
  // spec.hs:349:1-63
  wire [0:0] i_4;
  // spec.hs:349:1-63
  wire [5:0] ws_4;
  wire [143:0] result_36;
  // spec.hs:112:34-42
  wire  x0_9;
  // spec.hs:112:34-42
  wire  x1_9;
  // spec.hs:(669,1)-(674,36)
  reg [143:0] result_37 = {{8'd13,   64'sd0},   {8'd13,   64'sd0}};
  // spec.hs:(669,1)-(674,36)
  wire [143:0] t_5;
  wire signed [63:0] x_6;
  wire signed [63:0] y_4;
  // spec.hs:(393,1)-(395,59)
  wire signed [63:0] c$out0_case_alt;
  // spec.hs:362:1-53
  wire signed [63:0] dta_1;
  // spec.hs:362:1-53
  wire [7:0] tag_2;
  // spec.hs:362:1-53
  wire [7:0] tagToMatch_0;
  // spec.hs:(393,1)-(395,59)
  wire signed [63:0] c$out0_case_alt_0;
  // spec.hs:362:1-53
  wire signed [63:0] dta_2;
  // spec.hs:362:1-53
  wire [7:0] tagToMatch_1;
  // spec.hs:362:1-53
  wire [7:0] tag_3;
  wire [71:0] result_38;
  // spec.hs:(658,1)-(662,28)
  reg [71:0] result_39 = {8'd13,   64'sd0};
  // spec.hs:(658,1)-(662,28)
  wire  b_1;
  // spec.hs:(393,1)-(395,59)
  reg signed [63:0] c$input1Win_app_arg = (64'sd0);
  wire [71:0] result_40;
  // spec.hs:(648,1)-(652,28)
  reg [71:0] result_41 = {8'd13,   64'sd0};
  // spec.hs:(648,1)-(652,28)
  wire  b_2;
  // spec.hs:(393,1)-(395,59)
  reg signed [63:0] c$input0Win_app_arg = (64'sd0);
  // spec.hs:(393,1)-(395,59)
  wire [129:0] inputs_2;
  wire [6:0] c$app_arg_11;
  wire  result_42;
  wire [503:0] c$app_arg_12;
  wire [71:0] result_43;
  wire [287:0] c$app_arg_13;
  wire [71:0] result_44;
  wire [215:0] c$app_arg_14;
  wire [71:0] result_45;
  wire [143:0] c$app_arg_15;
  wire [71:0] result_46;
  // spec.hs:(627,9)-(629,29)
  reg [7:0] t_6 = 8'd1;
  wire [7:0] result_47;
  // spec.hs:(627,9)-(629,29)
  wire  b_3;
  // spec.hs:(627,9)-(629,29)
  wire [7:0] f1;
  wire [7:0] result_48;
  // spec.hs:(393,1)-(395,59)
  wire  pOut5;
  // spec.hs:(627,9)-(629,29)
  reg [7:0] t_7 = 8'd1;
  wire [7:0] result_49;
  // spec.hs:(627,9)-(629,29)
  wire  b_4;
  // spec.hs:(627,9)-(629,29)
  wire [7:0] f1_0;
  wire [7:0] result_50;
  // spec.hs:(393,1)-(395,59)
  wire  c$ds8_case_alt;
  // spec.hs:116:34-42
  wire  x1_10;
  // spec.hs:116:34-42
  wire  x0_10;
  // spec.hs:(627,9)-(629,29)
  reg [7:0] t_8 = 8'd1;
  wire [7:0] result_51;
  // spec.hs:(627,9)-(629,29)
  wire  b_5;
  // spec.hs:(627,9)-(629,29)
  wire [7:0] f1_1;
  wire [7:0] result_52;
  // spec.hs:(393,1)-(395,59)
  wire  c$ds7_case_alt;
  // spec.hs:115:34-42
  wire  x1_11;
  // spec.hs:115:34-42
  wire  x0_11;
  // spec.hs:(627,9)-(629,29)
  reg [7:0] t_9 = 8'd1;
  wire [7:0] result_53;
  // spec.hs:(627,9)-(629,29)
  wire  b_6;
  // spec.hs:(627,9)-(629,29)
  wire [7:0] f1_2;
  wire [7:0] result_54;
  // spec.hs:(393,1)-(395,59)
  wire  c$ds6_case_alt;
  // spec.hs:114:34-42
  wire  x1_12;
  // spec.hs:114:34-42
  wire  x0_12;
  // spec.hs:(627,9)-(629,29)
  reg [7:0] t_10 = 8'd1;
  wire [7:0] result_55;
  // spec.hs:(627,9)-(629,29)
  wire  b_7;
  // spec.hs:(627,9)-(629,29)
  wire [7:0] f1_3;
  wire [7:0] result_56;
  // spec.hs:(393,1)-(395,59)
  wire  c$ds5_case_alt;
  // spec.hs:113:34-42
  wire  x1_13;
  // spec.hs:113:34-42
  wire  x0_13;
  // spec.hs:(627,9)-(629,29)
  reg [7:0] t_11 = 8'd1;
  wire [7:0] result_57;
  // spec.hs:(627,9)-(629,29)
  wire  b_8;
  // spec.hs:(627,9)-(629,29)
  wire [7:0] f1_4;
  wire [7:0] result_58;
  // spec.hs:(393,1)-(395,59)
  wire  c$ds4_case_alt;
  // spec.hs:112:34-42
  wire  x1_14;
  // spec.hs:112:34-42
  wire  x0_14;
  wire [9:0] c$app_arg_16;
  wire [1:0] result_59;
  // spec.hs:(393,1)-(395,59)
  wire [1:0] pOut4;
  wire [7:0] c$app_arg_17;
  wire [1:0] result_60;
  // spec.hs:(393,1)-(395,59)
  wire [1:0] pOut3;
  wire [5:0] c$app_arg_18;
  wire [1:0] result_61;
  // spec.hs:(393,1)-(395,59)
  wire [1:0] pOut2;
  wire [5:0] c$app_arg_19;
  wire [1:0] result_62;
  // spec.hs:(393,1)-(395,59)
  wire [1:0] pOut1;
  wire [5:0] c$app_arg_20;
  wire [1:0] result_63;
  // spec.hs:(393,1)-(395,59)
  wire [1:0] pOut0;
  wire [1:0] c$app_arg_21;
  wire  result_64;
  // spec.hs:(627,9)-(629,29)
  reg [7:0] t_12 = 8'd1;
  wire [7:0] result_65;
  // spec.hs:(627,9)-(629,29)
  wire  b_9;
  // spec.hs:(627,9)-(629,29)
  wire [7:0] f1_5;
  wire [7:0] result_66;
  // spec.hs:(393,1)-(395,59)
  wire  pIn1_0;
  wire [1:0] c$app_arg_22;
  wire  result_67;
  // spec.hs:(627,9)-(629,29)
  reg [7:0] t_13 = 8'd1;
  wire [7:0] result_68;
  // spec.hs:(627,9)-(629,29)
  wire  b_10;
  // spec.hs:(627,9)-(629,29)
  wire [7:0] f1_6;
  wire [7:0] result_69;
  // spec.hs:(393,1)-(395,59)
  wire  pIn0_0;
  // spec.hs:(393,1)-(395,59)
  wire [12:0] pacings;
  // spec.hs:(636,1)-(638,22)
  reg signed [63:0] toWait = (64'sd0);
  wire  result_70;
  wire signed [63:0] result_71;
  // spec.hs:(636,1)-(638,22)
  wire  b_11;
  wire signed [63:0] result_72;
  wire signed [63:0] x_7;
  wire  result_73;
  // spec.hs:(393,1)-(395,59)
  wire  b_12;
  wire [145:0] result_74;
  reg [143:0] c$app_arg_23 = {{{64'sd0,   1'b0},   {64'sd0,   1'b0}},   1'b0,   {1'b0,   1'b0,   {1'b0,
                                                                     1'b0},
                                                    {1'b0,   1'b0},   {1'b0,
                                                                       1'b0},
                                                    {1'b0,   1'b0},   {1'b0,
                                                                       1'b0},
                                                    1'b0}};
  wire [143:0] c$case_alt;
  wire [143:0] c$case_alt_0;
  wire [143:0] c$case_alt_1;
  reg [143:0] c$case_alt_2;
  reg [143:0] c$case_alt_3;
  wire [143:0] c$case_alt_4;
  reg  c$app_arg_24 = 1'b0;
  wire  c$case_alt_5;
  wire  c$case_alt_6;
  wire  c$case_alt_7;
  wire  c$case_alt_8;
  reg  c$case_alt_9;
  reg  c$app_arg_25 = 1'b0;
  wire  c$case_alt_10;
  wire  c$case_alt_11;
  wire  c$case_alt_12;
  // spec.hs:(202,1)-(204,25)
  reg [719:0] buffer = {{{{64'sd0,   1'b0},   {64'sd0,   1'b0}},   1'b0,   {1'b0,   1'b0,   {1'b0,
                                                                      1'b0},
                                                     {1'b0,   1'b0},   {1'b0,
                                                                        1'b0},
                                                     {1'b0,   1'b0},   {1'b0,
                                                                        1'b0},
                                                     1'b0}},   {{{64'sd0,
                                                                  1'b0},
                                                                 {64'sd0,
                                                                  1'b0}},
                                                                1'b0,   {1'b0,
                                                                         1'b0,
                                                                         {1'b0,
                                                                          1'b0},
                                                                         {1'b0,
                                                                          1'b0},
                                                                         {1'b0,
                                                                          1'b0},
                                                                         {1'b0,
                                                                          1'b0},
                                                                         {1'b0,
                                                                          1'b0},
                                                                         1'b0}},
 {{{64'sd0,   1'b0},   {64'sd0,   1'b0}},   1'b0,   {1'b0,   1'b0,   {1'b0,
                                                                      1'b0},
                                                     {1'b0,   1'b0},   {1'b0,
                                                                        1'b0},
                                                     {1'b0,   1'b0},   {1'b0,
                                                                        1'b0},
                                                     1'b0}},   {{{64'sd0,
                                                                  1'b0},
                                                                 {64'sd0,
                                                                  1'b0}},
                                                                1'b0,   {1'b0,
                                                                         1'b0,
                                                                         {1'b0,
                                                                          1'b0},
                                                                         {1'b0,
                                                                          1'b0},
                                                                         {1'b0,
                                                                          1'b0},
                                                                         {1'b0,
                                                                          1'b0},
                                                                         {1'b0,
                                                                          1'b0},
                                                                         1'b0}},
 {{{64'sd0,   1'b0},   {64'sd0,   1'b0}},   1'b0,   {1'b0,   1'b0,   {1'b0,
                                                                      1'b0},
                                                     {1'b0,   1'b0},   {1'b0,
                                                                        1'b0},
                                                     {1'b0,   1'b0},   {1'b0,
                                                                        1'b0},
                                                     1'b0}}};
  // spec.hs:(202,1)-(204,25)
  wire [719:0] c$buffer_case_alt;
  // spec.hs:(202,1)-(204,25)
  wire [719:0] c$buffer_case_alt_0;
  // spec.hs:(202,1)-(204,25)
  wire [719:0] c$buffer_case_alt_1;
  // spec.hs:(202,1)-(204,25)
  wire [143:0] qData;
  // spec.hs:(202,1)-(204,25)
  wire signed [63:0] x_8;
  // spec.hs:(202,1)-(204,25)
  reg signed [63:0] cursor = (64'sd0);
  // spec.hs:(202,1)-(204,25)
  wire signed [63:0] c$cursor_case_alt;
  // spec.hs:(202,1)-(204,25)
  wire signed [63:0] c$cursor_case_alt_0;
  // spec.hs:(202,1)-(204,25)
  wire signed [63:0] c$cursor_case_alt_1;
  // spec.hs:(202,1)-(204,25)
  wire  c$cursor_case_scrut;
  // spec.hs:(202,1)-(204,25)
  wire signed [63:0] c$cursor_case_alt_2;
  // spec.hs:(202,1)-(204,25)
  reg signed [63:0] c$cursor_case_alt_3;
  // spec.hs:(202,1)-(204,25)
  wire signed [63:0] c$cursor_case_alt_4;
  // spec.hs:(801,1)-(803,87)
  wire  qPopValid;
  // spec.hs:(801,1)-(803,87)
  wire  qPush;
  // spec.hs:(801,1)-(803,87)
  wire  qPop;
  wire [129:0] inputs;
  wire [3:0] c$vec2;
  wire [215:0] t_projection_0;
  wire [8:0] c$vec2_0;
  wire [8:0] c$vec2_1;
  wire [3:0] c$vec2_2;
  wire [8:0] c$vec2_3;
  wire [287:0] t_projection_5;
  wire signed [63:0] x_projection_6;
  wire signed [63:0] x_projection_9;
  wire [703:0] c$vec;
  wire signed [63:0] x_projection_19;
  wire [767:0] c$t_case_alt_sel_alt_t_0;
  wire [767:0] c$t_case_alt_sel_alt_f_0;
  wire [3:0] c$vec2_4;
  wire [215:0] t_projection_7;
  wire signed [63:0] c$tte_rhs;
  wire [863:0] c$buffer_case_alt_sel_alt_t_1;
  wire [863:0] c$buffer_case_alt_sel_alt_f_2;
  wire signed [63:0] c$tte_rhs_0;
  wire [339:0] result;
  wire [326:0] result_0;
  wire [64:0] result_0_0;
  wire [64:0] result_0_1;
  wire [64:0] result_0_2;
  wire [64:0] result_0_3;
  wire [1:0] result_0_4;
  wire [64:0] result_0_5;
  wire [12:0] result_1;
  wire [7:0] result_1_5;

  assign inputs = {{inputs_0_0,   inputs_0_1},
                   {inputs_1_0,   inputs_1_1}};

  assign result_2 = timer0Over ? (64'sd2000) : ((x + 64'sd2000));

  assign x = result_3;

  // register begin
  always @(posedge clk or  posedge  rst) begin : result_3_register
    if ( rst) begin
      result_3 <= (64'sd0);
    end else if (en) begin
      result_3 <= result_2;
    end
  end
  // register end

  assign result_4 = {inputs[65:65] | (inputs[0:0] | timer0Over),
                     {inputs,   timer0Over,   {pIn0,   pIn1,
                                               {pIn0,   pIn1},   {pIn0,   pIn1},   {pIn0,
                                                                                    pIn1},   {pIn0,   pIn1},   {pIn0,
                                                                                                                pIn1},
                                               timer0Over}}};

  assign pIn0 = inputs[65:65];

  assign pIn1 = inputs[0:0];

  assign timer0Over = result_3 >= (64'sd1000000);

  assign result_5 = {{result_70 & (~ result_73),
                      {{result_17,   result_18},   {result_15,
                                                    result_16},   {result_9,   result_10},
                       {dta_0,   result_8},   {result_29[0:0],
                                               result_7},   {dta,   result_6}}},   {slides,
                                                                                    {result_67,   result_64,
                                                                                     x0_3 & x1_3,   x0_2 & x1_2,
                                                                                     x0_1 & x1_1,   x0_0 & x1_0,
                                                                                     x0 & x1,   result_42}}};

  assign x1 = result_59[0:0];

  assign x0 = result_59[1:1];

  assign x1_0 = result_60[0:0];

  assign x0_0 = result_60[1:1];

  assign x1_1 = result_61[0:0];

  assign x0_1 = result_61[1:1];

  assign x1_2 = result_62[0:0];

  assign x0_2 = result_62[1:1];

  assign x1_3 = result_63[0:0];

  assign x0_3 = result_63[1:1];

  wire  iterateI_ho1_0_arg0;
  wire  iterateI_ho1_0_res;
  wire  iterateI_ho1_1_res;
  wire  iterateI_ho1_2_res;
  wire  iterateI_ho1_3_res;
  wire  iterateI_ho1_4_res;
  wire  iterateI_ho1_5_res;
  wire  iterateI_ho1_6_res;
  assign iterateI_ho1_0_arg0 = pOut5;

  reg  c$bb_res_res = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_delay
    if (en) begin
      c$bb_res_res <= iterateI_ho1_0_arg0;
    end
  end
  // delay end

  assign iterateI_ho1_0_res = c$bb_res_res;



  reg  c$bb_res_res_0 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_0_delay
    if (en) begin
      c$bb_res_res_0 <= iterateI_ho1_0_res;
    end
  end
  // delay end

  assign iterateI_ho1_1_res = c$bb_res_res_0;



  reg  c$bb_res_res_1 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_1_delay
    if (en) begin
      c$bb_res_res_1 <= iterateI_ho1_1_res;
    end
  end
  // delay end

  assign iterateI_ho1_2_res = c$bb_res_res_1;



  reg  c$bb_res_res_2 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_2_delay
    if (en) begin
      c$bb_res_res_2 <= iterateI_ho1_2_res;
    end
  end
  // delay end

  assign iterateI_ho1_3_res = c$bb_res_res_2;



  reg  c$bb_res_res_3 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_3_delay
    if (en) begin
      c$bb_res_res_3 <= iterateI_ho1_3_res;
    end
  end
  // delay end

  assign iterateI_ho1_4_res = c$bb_res_res_3;



  reg  c$bb_res_res_4 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_4_delay
    if (en) begin
      c$bb_res_res_4 <= iterateI_ho1_4_res;
    end
  end
  // delay end

  assign iterateI_ho1_5_res = c$bb_res_res_4;



  reg  c$bb_res_res_5 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_5_delay
    if (en) begin
      c$bb_res_res_5 <= iterateI_ho1_5_res;
    end
  end
  // delay end

  assign iterateI_ho1_6_res = c$bb_res_res_5;



  assign c$app_arg = {pOut5,
                      iterateI_ho1_0_res,   iterateI_ho1_1_res,
                      iterateI_ho1_2_res,   iterateI_ho1_3_res,
                      iterateI_ho1_4_res,   iterateI_ho1_5_res,
                      iterateI_ho1_6_res};



  assign result_6 = c$app_arg[1-1:0];

  wire  iterateI_ho1_0_res_0;
  wire  iterateI_ho1_1_res_0;
  wire  iterateI_ho1_2_res_0;
  wire  iterateI_ho1_3_res_0;
  wire  iterateI_ho1_4_res_0;
  wire  iterateI_ho1_5_res_0;
  wire  iterateI_ho1_6_res_0;
  reg  c$bb_res_res_6 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_6_delay
    if (en) begin
      c$bb_res_res_6 <= c$ds8_case_alt;
    end
  end
  // delay end

  assign iterateI_ho1_0_res_0 = c$bb_res_res_6;



  reg  c$bb_res_res_7 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_7_delay
    if (en) begin
      c$bb_res_res_7 <= iterateI_ho1_0_res_0;
    end
  end
  // delay end

  assign iterateI_ho1_1_res_0 = c$bb_res_res_7;



  reg  c$bb_res_res_8 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_8_delay
    if (en) begin
      c$bb_res_res_8 <= iterateI_ho1_1_res_0;
    end
  end
  // delay end

  assign iterateI_ho1_2_res_0 = c$bb_res_res_8;



  reg  c$bb_res_res_9 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_9_delay
    if (en) begin
      c$bb_res_res_9 <= iterateI_ho1_2_res_0;
    end
  end
  // delay end

  assign iterateI_ho1_3_res_0 = c$bb_res_res_9;



  reg  c$bb_res_res_10 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_10_delay
    if (en) begin
      c$bb_res_res_10 <= iterateI_ho1_3_res_0;
    end
  end
  // delay end

  assign iterateI_ho1_4_res_0 = c$bb_res_res_10;



  reg  c$bb_res_res_11 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_11_delay
    if (en) begin
      c$bb_res_res_11 <= iterateI_ho1_4_res_0;
    end
  end
  // delay end

  assign iterateI_ho1_5_res_0 = c$bb_res_res_11;



  reg  c$bb_res_res_12 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_12_delay
    if (en) begin
      c$bb_res_res_12 <= iterateI_ho1_5_res_0;
    end
  end
  // delay end

  assign iterateI_ho1_6_res_0 = c$bb_res_res_12;



  assign c$app_arg_0 = {c$ds8_case_alt,
                        iterateI_ho1_0_res_0,   iterateI_ho1_1_res_0,
                        iterateI_ho1_2_res_0,   iterateI_ho1_3_res_0,
                        iterateI_ho1_4_res_0,   iterateI_ho1_5_res_0,
                        iterateI_ho1_6_res_0};



  assign result_7 = c$app_arg_0[1-1:0];

  wire  iterateI_ho1_0_res_1;
  wire  iterateI_ho1_1_res_1;
  wire  iterateI_ho1_2_res_1;
  wire  iterateI_ho1_3_res_1;
  wire  iterateI_ho1_4_res_1;
  wire  iterateI_ho1_5_res_1;
  wire  iterateI_ho1_6_res_1;
  reg  c$bb_res_res_13 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_13_delay
    if (en) begin
      c$bb_res_res_13 <= c$ds7_case_alt;
    end
  end
  // delay end

  assign iterateI_ho1_0_res_1 = c$bb_res_res_13;



  reg  c$bb_res_res_14 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_14_delay
    if (en) begin
      c$bb_res_res_14 <= iterateI_ho1_0_res_1;
    end
  end
  // delay end

  assign iterateI_ho1_1_res_1 = c$bb_res_res_14;



  reg  c$bb_res_res_15 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_15_delay
    if (en) begin
      c$bb_res_res_15 <= iterateI_ho1_1_res_1;
    end
  end
  // delay end

  assign iterateI_ho1_2_res_1 = c$bb_res_res_15;



  reg  c$bb_res_res_16 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_16_delay
    if (en) begin
      c$bb_res_res_16 <= iterateI_ho1_2_res_1;
    end
  end
  // delay end

  assign iterateI_ho1_3_res_1 = c$bb_res_res_16;



  reg  c$bb_res_res_17 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_17_delay
    if (en) begin
      c$bb_res_res_17 <= iterateI_ho1_3_res_1;
    end
  end
  // delay end

  assign iterateI_ho1_4_res_1 = c$bb_res_res_17;



  reg  c$bb_res_res_18 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_18_delay
    if (en) begin
      c$bb_res_res_18 <= iterateI_ho1_4_res_1;
    end
  end
  // delay end

  assign iterateI_ho1_5_res_1 = c$bb_res_res_18;



  reg  c$bb_res_res_19 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_19_delay
    if (en) begin
      c$bb_res_res_19 <= iterateI_ho1_5_res_1;
    end
  end
  // delay end

  assign iterateI_ho1_6_res_1 = c$bb_res_res_19;



  assign c$app_arg_1 = {c$ds7_case_alt,
                        iterateI_ho1_0_res_1,   iterateI_ho1_1_res_1,
                        iterateI_ho1_2_res_1,   iterateI_ho1_3_res_1,
                        iterateI_ho1_4_res_1,   iterateI_ho1_5_res_1,
                        iterateI_ho1_6_res_1};



  assign result_8 = c$app_arg_1[1-1:0];

  assign c$vec2 = (ws[4-1 : 0]);

  // zipWith start
  genvar i_5;
  generate
  for (i_5 = 0; i_5 < 2; i_5 = i_5 + 1) begin : zipWith
    wire [71:0] zipWith_in1;
    assign zipWith_in1 = result_12[i_5*72+:72];
    wire [1:0] zipWith_in2;
    assign zipWith_in2 = c$vec2[i_5*2+:2];
    wire [73:0] c$n;
    assign c$n = {zipWith_in1,   zipWith_in2};


    assign c$ws_app_arg[i_5*74+:74] = c$n;
  end
  endgenerate
  // zipWith end

  // imap begin
  genvar i_6;
  generate
  for (i_6=0; i_6 < 2; i_6 = i_6 + 1) begin : imap
    wire [1-1:0] map_index;
    wire [73:0] map_in;
    assign map_in = c$ws_app_arg[i_6*74+:74];
    wire [1:0] map_out;

    assign map_index = 1'd1 - i_6[0+:1];
    wire [1:0] c$case_alt_13;
    // spec.hs:349:1-63
    wire [7:0] t_14;
    // spec.hs:349:1-63
    wire [71:0] x_10;
    assign map_out = c$case_alt_13;

    assign c$case_alt_13 = (t_14 == result_43[39:32]) ? {1'b1,map_index} : map_in[1:0];

    assign t_14 = x_10[71:64];

    assign x_10 = map_in[73:2];


    assign c$ws_app_arg_0[i_6*2+:2] = map_out;
  end
  endgenerate
  // imap end

  // index begin
  wire [71:0] vecArray [0:2-1];
  genvar i_7;
  generate
  for (i_7=0; i_7 < 2; i_7=i_7+1) begin : mk_array
    assign vecArray[(2-1)-i_7] = result_12[i_7*72+:72];
  end
  endgenerate
  assign c$case_scrut = vecArray[($unsigned({{(64-1) {1'b0}},i}))];
  // index end

  assign c$case_scrut_0 = ws[6-1 -: 2];

  assign result_9 = c$case_scrut_0[1:1] ? $signed(c$case_scrut[63:0]) : (64'sd0);

  assign i = c$case_scrut_0[0:0];

  assign ws = {c$ws_app_arg_0,{1'b0,1'bx}};

  wire  iterateI_ho1_0_res_2;
  wire  iterateI_ho1_1_res_2;
  wire  iterateI_ho1_2_res_2;
  wire  iterateI_ho1_3_res_2;
  wire  iterateI_ho1_4_res_2;
  wire  iterateI_ho1_5_res_2;
  wire  iterateI_ho1_6_res_2;
  reg  c$bb_res_res_20 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_20_delay
    if (en) begin
      c$bb_res_res_20 <= c$ds6_case_alt;
    end
  end
  // delay end

  assign iterateI_ho1_0_res_2 = c$bb_res_res_20;



  reg  c$bb_res_res_21 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_21_delay
    if (en) begin
      c$bb_res_res_21 <= iterateI_ho1_0_res_2;
    end
  end
  // delay end

  assign iterateI_ho1_1_res_2 = c$bb_res_res_21;



  reg  c$bb_res_res_22 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_22_delay
    if (en) begin
      c$bb_res_res_22 <= iterateI_ho1_1_res_2;
    end
  end
  // delay end

  assign iterateI_ho1_2_res_2 = c$bb_res_res_22;



  reg  c$bb_res_res_23 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_23_delay
    if (en) begin
      c$bb_res_res_23 <= iterateI_ho1_2_res_2;
    end
  end
  // delay end

  assign iterateI_ho1_3_res_2 = c$bb_res_res_23;



  reg  c$bb_res_res_24 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_24_delay
    if (en) begin
      c$bb_res_res_24 <= iterateI_ho1_3_res_2;
    end
  end
  // delay end

  assign iterateI_ho1_4_res_2 = c$bb_res_res_24;



  reg  c$bb_res_res_25 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_25_delay
    if (en) begin
      c$bb_res_res_25 <= iterateI_ho1_4_res_2;
    end
  end
  // delay end

  assign iterateI_ho1_5_res_2 = c$bb_res_res_25;



  reg  c$bb_res_res_26 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_26_delay
    if (en) begin
      c$bb_res_res_26 <= iterateI_ho1_5_res_2;
    end
  end
  // delay end

  assign iterateI_ho1_6_res_2 = c$bb_res_res_26;



  assign c$app_arg_2 = {c$ds6_case_alt,
                        iterateI_ho1_0_res_2,   iterateI_ho1_1_res_2,
                        iterateI_ho1_2_res_2,   iterateI_ho1_3_res_2,
                        iterateI_ho1_4_res_2,   iterateI_ho1_5_res_2,
                        iterateI_ho1_6_res_2};



  assign result_10 = c$app_arg_2[1-1:0];

  assign result_11 = (x0_4 & x1_4) ? t : result_12;

  assign x0_4 = result_61[1:1];

  assign x1_4 = result_61[0:0];

  // register begin
  always @(posedge clk or  posedge  rst) begin : result_12_register
    if ( rst) begin
      result_12 <= {{8'd13,   64'sd0},   {8'd13,   64'sd0}};
    end else if (en) begin
      result_12 <= result_11;
    end
  end
  // register end

  assign t_projection_0 = ({result_12,{result_46[39:32],   (x_0 + y)}});

  assign t = t_projection_0[143:0];

  assign x_0 = c$out0_case_alt_0;

  assign y = result_13;

  assign c$vec2_0 = (ws_0[9-1 : 0]);

  // zipWith start
  genvar i_9;
  generate
  for (i_9 = 0; i_9 < 3; i_9 = i_9 + 1) begin : zipWith_0
    wire [71:0] zipWith_in1_0;
    assign zipWith_in1_0 = result_21[i_9*72+:72];
    wire [2:0] zipWith_in2_0;
    assign zipWith_in2_0 = c$vec2_0[i_9*3+:3];
    wire [74:0] c$n_0;
    assign c$n_0 = {zipWith_in1_0,   zipWith_in2_0};


    assign c$ws_app_arg_1[i_9*75+:75] = c$n_0;
  end
  endgenerate
  // zipWith end

  // imap begin
  genvar i_10;
  generate
  for (i_10=0; i_10 < 3; i_10 = i_10 + 1) begin : imap_0
    wire [2-1:0] map_index_0;
    wire [74:0] map_in_0;
    assign map_in_0 = c$ws_app_arg_1[i_10*75+:75];
    wire [2:0] map_out_0;

    assign map_index_0 = 2'd2 - i_10[0+:2];
    wire [2:0] c$case_alt_14;
    // spec.hs:341:1-65
    wire [7:0] t_16;
    // spec.hs:341:1-65
    wire [71:0] x_11;
    assign map_out_0 = c$case_alt_14;

    assign c$case_alt_14 = (t_16 == result_14) ? {1'b1,map_index_0} : map_in_0[2:0];

    assign t_16 = x_11[71:64];

    assign x_11 = map_in_0[74:3];


    assign c$ws_app_arg_2[i_10*3+:3] = map_out_0;
  end
  endgenerate
  // imap end

  // index begin
  wire [71:0] vecArray_0 [0:3-1];
  genvar i_11;
  generate
  for (i_11=0; i_11 < 3; i_11=i_11+1) begin : mk_array_0
    assign vecArray_0[(3-1)-i_11] = result_21[i_11*72+:72];
  end
  endgenerate
  assign c$case_scrut_1 = vecArray_0[($unsigned({{(64-2) {1'b0}},i_0}))];
  // index end

  assign c$case_scrut_2 = ws_0[12-1 -: 3];

  assign result_13 = c$case_scrut_2[2:2] ? $signed(c$case_scrut_1[63:0]) : (64'sd0);

  assign result_14 = (result_46[47:40] > 8'd3) ? c$app_arg_3 : (c$app_arg_3 + 8'd12);

  assign c$app_arg_3 = result_46[47:40] - 8'd3;

  assign i_0 = c$case_scrut_2[1:0];

  assign ws_0 = {c$ws_app_arg_2,{1'b0,2'bxx}};

  assign c$vec2_1 = (ws_1[9-1 : 0]);

  // zipWith start
  genvar i_12;
  generate
  for (i_12 = 0; i_12 < 3; i_12 = i_12 + 1) begin : zipWith_1
    wire [71:0] zipWith_in1_1;
    assign zipWith_in1_1 = result_21[i_12*72+:72];
    wire [2:0] zipWith_in2_1;
    assign zipWith_in2_1 = c$vec2_1[i_12*3+:3];
    wire [74:0] c$n_1;
    assign c$n_1 = {zipWith_in1_1,   zipWith_in2_1};


    assign c$ws_app_arg_3[i_12*75+:75] = c$n_1;
  end
  endgenerate
  // zipWith end

  // imap begin
  genvar i_13;
  generate
  for (i_13=0; i_13 < 3; i_13 = i_13 + 1) begin : imap_1
    wire [2-1:0] map_index_1;
    wire [74:0] map_in_1;
    assign map_in_1 = c$ws_app_arg_3[i_13*75+:75];
    wire [2:0] map_out_1;

    assign map_index_1 = 2'd2 - i_13[0+:2];
    wire [2:0] c$case_alt_15;
    // spec.hs:341:1-65
    wire [7:0] t_17;
    // spec.hs:341:1-65
    wire [71:0] x_12;
    assign map_out_1 = c$case_alt_15;

    assign c$case_alt_15 = (t_17 == result_43[47:40]) ? {1'b1,map_index_1} : map_in_1[2:0];

    assign t_17 = x_12[71:64];

    assign x_12 = map_in_1[74:3];


    assign c$ws_app_arg_4[i_13*3+:3] = map_out_1;
  end
  endgenerate
  // imap end

  // index begin
  wire [71:0] vecArray_1 [0:3-1];
  genvar i_14;
  generate
  for (i_14=0; i_14 < 3; i_14=i_14+1) begin : mk_array_1
    assign vecArray_1[(3-1)-i_14] = result_21[i_14*72+:72];
  end
  endgenerate
  assign c$case_scrut_3 = vecArray_1[($unsigned({{(64-2) {1'b0}},i_1}))];
  // index end

  assign c$case_scrut_4 = ws_1[12-1 -: 3];

  assign result_15 = c$case_scrut_4[2:2] ? $signed(c$case_scrut_3[63:0]) : (64'sd0);

  assign i_1 = c$case_scrut_4[1:0];

  assign ws_1 = {c$ws_app_arg_4,{1'b0,2'bxx}};

  wire  iterateI_ho1_0_res_3;
  wire  iterateI_ho1_1_res_3;
  wire  iterateI_ho1_2_res_3;
  wire  iterateI_ho1_3_res_3;
  wire  iterateI_ho1_4_res_3;
  wire  iterateI_ho1_5_res_3;
  wire  iterateI_ho1_6_res_3;
  reg  c$bb_res_res_27 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_27_delay
    if (en) begin
      c$bb_res_res_27 <= c$ds5_case_alt;
    end
  end
  // delay end

  assign iterateI_ho1_0_res_3 = c$bb_res_res_27;



  reg  c$bb_res_res_28 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_28_delay
    if (en) begin
      c$bb_res_res_28 <= iterateI_ho1_0_res_3;
    end
  end
  // delay end

  assign iterateI_ho1_1_res_3 = c$bb_res_res_28;



  reg  c$bb_res_res_29 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_29_delay
    if (en) begin
      c$bb_res_res_29 <= iterateI_ho1_1_res_3;
    end
  end
  // delay end

  assign iterateI_ho1_2_res_3 = c$bb_res_res_29;



  reg  c$bb_res_res_30 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_30_delay
    if (en) begin
      c$bb_res_res_30 <= iterateI_ho1_2_res_3;
    end
  end
  // delay end

  assign iterateI_ho1_3_res_3 = c$bb_res_res_30;



  reg  c$bb_res_res_31 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_31_delay
    if (en) begin
      c$bb_res_res_31 <= iterateI_ho1_3_res_3;
    end
  end
  // delay end

  assign iterateI_ho1_4_res_3 = c$bb_res_res_31;



  reg  c$bb_res_res_32 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_32_delay
    if (en) begin
      c$bb_res_res_32 <= iterateI_ho1_4_res_3;
    end
  end
  // delay end

  assign iterateI_ho1_5_res_3 = c$bb_res_res_32;



  reg  c$bb_res_res_33 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_33_delay
    if (en) begin
      c$bb_res_res_33 <= iterateI_ho1_5_res_3;
    end
  end
  // delay end

  assign iterateI_ho1_6_res_3 = c$bb_res_res_33;



  assign c$app_arg_4 = {c$ds5_case_alt,
                        iterateI_ho1_0_res_3,   iterateI_ho1_1_res_3,
                        iterateI_ho1_2_res_3,   iterateI_ho1_3_res_3,
                        iterateI_ho1_4_res_3,   iterateI_ho1_5_res_3,
                        iterateI_ho1_6_res_3};



  assign result_16 = c$app_arg_4[1-1:0];

  assign c$vec2_2 = (ws_2[4-1 : 0]);

  // zipWith start
  genvar i_15;
  generate
  for (i_15 = 0; i_15 < 2; i_15 = i_15 + 1) begin : zipWith_2
    wire [71:0] zipWith_in1_2;
    assign zipWith_in1_2 = result_37[i_15*72+:72];
    wire [1:0] zipWith_in2_2;
    assign zipWith_in2_2 = c$vec2_2[i_15*2+:2];
    wire [73:0] c$n_2;
    assign c$n_2 = {zipWith_in1_2,   zipWith_in2_2};


    assign c$ws_app_arg_5[i_15*74+:74] = c$n_2;
  end
  endgenerate
  // zipWith end

  // imap begin
  genvar i_16;
  generate
  for (i_16=0; i_16 < 2; i_16 = i_16 + 1) begin : imap_2
    wire [1-1:0] map_index_2;
    wire [73:0] map_in_2;
    assign map_in_2 = c$ws_app_arg_5[i_16*74+:74];
    wire [1:0] map_out_2;

    assign map_index_2 = 1'd1 - i_16[0+:1];
    wire [1:0] c$case_alt_16;
    // spec.hs:349:1-63
    wire [7:0] t_18;
    // spec.hs:349:1-63
    wire [71:0] x_13;
    assign map_out_2 = c$case_alt_16;

    assign c$case_alt_16 = (t_18 == result_43[55:48]) ? {1'b1,map_index_2} : map_in_2[1:0];

    assign t_18 = x_13[71:64];

    assign x_13 = map_in_2[73:2];


    assign c$ws_app_arg_6[i_16*2+:2] = map_out_2;
  end
  endgenerate
  // imap end

  // index begin
  wire [71:0] vecArray_2 [0:2-1];
  genvar i_17;
  generate
  for (i_17=0; i_17 < 2; i_17=i_17+1) begin : mk_array_2
    assign vecArray_2[(2-1)-i_17] = result_37[i_17*72+:72];
  end
  endgenerate
  assign c$case_scrut_5 = vecArray_2[($unsigned({{(64-1) {1'b0}},i_2}))];
  // index end

  assign c$case_scrut_6 = ws_2[6-1 -: 2];

  assign result_17 = c$case_scrut_6[1:1] ? $signed(c$case_scrut_5[63:0]) : (64'sd0);

  assign i_2 = c$case_scrut_6[0:0];

  assign ws_2 = {c$ws_app_arg_6,{1'b0,1'bx}};

  wire  iterateI_ho1_0_res_4;
  wire  iterateI_ho1_1_res_4;
  wire  iterateI_ho1_2_res_4;
  wire  iterateI_ho1_3_res_4;
  wire  iterateI_ho1_4_res_4;
  wire  iterateI_ho1_5_res_4;
  wire  iterateI_ho1_6_res_4;
  reg  c$bb_res_res_34 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_34_delay
    if (en) begin
      c$bb_res_res_34 <= c$ds4_case_alt;
    end
  end
  // delay end

  assign iterateI_ho1_0_res_4 = c$bb_res_res_34;



  reg  c$bb_res_res_35 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_35_delay
    if (en) begin
      c$bb_res_res_35 <= iterateI_ho1_0_res_4;
    end
  end
  // delay end

  assign iterateI_ho1_1_res_4 = c$bb_res_res_35;



  reg  c$bb_res_res_36 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_36_delay
    if (en) begin
      c$bb_res_res_36 <= iterateI_ho1_1_res_4;
    end
  end
  // delay end

  assign iterateI_ho1_2_res_4 = c$bb_res_res_36;



  reg  c$bb_res_res_37 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_37_delay
    if (en) begin
      c$bb_res_res_37 <= iterateI_ho1_2_res_4;
    end
  end
  // delay end

  assign iterateI_ho1_3_res_4 = c$bb_res_res_37;



  reg  c$bb_res_res_38 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_38_delay
    if (en) begin
      c$bb_res_res_38 <= iterateI_ho1_3_res_4;
    end
  end
  // delay end

  assign iterateI_ho1_4_res_4 = c$bb_res_res_38;



  reg  c$bb_res_res_39 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_39_delay
    if (en) begin
      c$bb_res_res_39 <= iterateI_ho1_4_res_4;
    end
  end
  // delay end

  assign iterateI_ho1_5_res_4 = c$bb_res_res_39;



  reg  c$bb_res_res_40 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_40_delay
    if (en) begin
      c$bb_res_res_40 <= iterateI_ho1_5_res_4;
    end
  end
  // delay end

  assign iterateI_ho1_6_res_4 = c$bb_res_res_40;



  assign c$app_arg_5 = {c$ds4_case_alt,
                        iterateI_ho1_0_res_4,   iterateI_ho1_1_res_4,
                        iterateI_ho1_2_res_4,   iterateI_ho1_3_res_4,
                        iterateI_ho1_4_res_4,   iterateI_ho1_5_res_4,
                        iterateI_ho1_6_res_4};



  assign result_18 = c$app_arg_5[1-1:0];

  assign c$vec2_3 = (ws_3[9-1 : 0]);

  // zipWith start
  genvar i_18;
  generate
  for (i_18 = 0; i_18 < 3; i_18 = i_18 + 1) begin : zipWith_3
    wire [71:0] zipWith_in1_3;
    assign zipWith_in1_3 = result_21[i_18*72+:72];
    wire [2:0] zipWith_in2_3;
    assign zipWith_in2_3 = c$vec2_3[i_18*3+:3];
    wire [74:0] c$n_3;
    assign c$n_3 = {zipWith_in1_3,   zipWith_in2_3};


    assign c$ws_app_arg_7[i_18*75+:75] = c$n_3;
  end
  endgenerate
  // zipWith end

  // imap begin
  genvar i_19;
  generate
  for (i_19=0; i_19 < 3; i_19 = i_19 + 1) begin : imap_3
    wire [2-1:0] map_index_3;
    wire [74:0] map_in_3;
    assign map_in_3 = c$ws_app_arg_7[i_19*75+:75];
    wire [2:0] map_out_3;

    assign map_index_3 = 2'd2 - i_19[0+:2];
    wire [2:0] c$case_alt_17;
    // spec.hs:341:1-65
    wire [7:0] t_19;
    // spec.hs:341:1-65
    wire [71:0] x_14;
    assign map_out_3 = c$case_alt_17;

    assign c$case_alt_17 = (t_19 == result_45[47:40]) ? {1'b1,map_index_3} : map_in_3[2:0];

    assign t_19 = x_14[71:64];

    assign x_14 = map_in_3[74:3];


    assign c$ws_app_arg_8[i_19*3+:3] = map_out_3;
  end
  endgenerate
  // imap end

  // index begin
  wire [71:0] vecArray_3 [0:3-1];
  genvar i_20;
  generate
  for (i_20=0; i_20 < 3; i_20=i_20+1) begin : mk_array_3
    assign vecArray_3[(3-1)-i_20] = result_21[i_20*72+:72];
  end
  endgenerate
  assign c$case_scrut_7 = vecArray_3[($unsigned({{(64-2) {1'b0}},i_3}))];
  // index end

  assign c$case_scrut_8 = ws_3[12-1 -: 3];

  assign result_19 = c$case_scrut_8[2:2] ? $signed(c$case_scrut_7[63:0]) : (64'sd0);

  assign i_3 = c$case_scrut_8[1:0];

  assign ws_3 = {c$ws_app_arg_8,{1'b0,2'bxx}};

  assign result_20 = (x0_5 & x1_5) ? t_0 : result_21;

  assign x0_5 = result_62[1:1];

  assign x1_5 = result_62[0:0];

  // register begin
  always @(posedge clk or  posedge  rst) begin : result_21_register
    if ( rst) begin
      result_21 <= {{8'd13,   64'sd0},   {8'd13,   64'sd0},   {8'd13,   64'sd0}};
    end else if (en) begin
      result_21 <= result_20;
    end
  end
  // register end

  assign t_projection_5 = ({result_21,{result_46[47:40],   (x_1 + y_0)}});

  assign t_0 = t_projection_5[215:0];

  assign x_projection_6 = (x_2 * y_1);

  assign x_1 = x_projection_6;

  assign y_0 = c$out1_case_alt;

  assign x_2 = c$out0_case_alt_0;

  assign y_1 = c$out0_case_alt;

  assign c$out1_case_alt = (result_34 == tag_0) ? dta_0 : (64'sd0);

  assign result_22 = (x0_6 & x1_6) ? t_1 : result_23;

  assign x0_6 = result_60[1:1];

  assign x1_6 = result_60[0:0];

  // register begin
  always @(posedge clk or  posedge  rst) begin : result_23_register
    if ( rst) begin
      result_23 <= {8'd13,   64'sd0};
    end else if (en) begin
      result_23 <= result_22;
    end
  end
  // register end

  assign t_1 = {result_45[31:24],   (x_4 + y_3)};

  assign x_3 = result_35;

  assign y_2 = result_19;

  assign x_projection_9 = (x_3 + y_2);

  assign x_4 = x_projection_9;

  assign y_3 = c$out3_case_alt;

  assign c$out3_case_alt = (tag == 8'd13) ? (64'sd0) : dta;

  assign dta = $signed(result_25[63:0]);

  assign tag = result_25[71:64];

  assign c$vec = window[703:0];

  wire [639:0] vec;
  wire signed [63:0] acc_4_0;
  wire signed [63:0] acc_1;
  wire signed [63:0] acc_2;
  wire signed [63:0] acc_3;
  wire signed [63:0] acc_4;
  wire signed [63:0] acc_5;
  wire signed [63:0] acc_6;
  wire signed [63:0] acc_7;
  wire signed [63:0] acc_8;
  wire signed [63:0] acc_9;
  wire signed [63:0] acc_10;
  wire signed [63:0] acc_1_0;
  wire signed [63:0] acc_1_1;
  wire signed [63:0] acc_1_2;
  wire signed [63:0] acc_1_3;
  wire signed [63:0] acc_2_0;
  wire signed [63:0] acc_2_1;
  wire signed [63:0] acc_3_0;
  wire signed [63:0] acc_1_4;
  assign c$t_app_arg = acc_4_0;

  assign vec = (c$vec[640-1 : 0]);

  assign acc_1 = $signed(vec[639:576]);

  assign acc_2 = $signed(vec[575:512]);

  assign acc_3 = $signed(vec[511:448]);

  assign acc_4 = $signed(vec[447:384]);

  assign acc_5 = $signed(vec[383:320]);

  assign acc_6 = $signed(vec[319:256]);

  assign acc_7 = $signed(vec[255:192]);

  assign acc_8 = $signed(vec[191:128]);

  assign acc_9 = $signed(vec[127:64]);

  assign acc_10 = $signed(vec[63:0]);

  wire signed [63:0] x_15;
  wire signed [63:0] y_6;
  wire signed [63:0] c$case_alt_18;
  assign acc_1_0 = c$case_alt_18;

  assign x_15 = acc_1;

  assign y_6 = acc_2;

  assign c$case_alt_18 = (x_15 + y_6);



  wire signed [63:0] x_16;
  wire signed [63:0] y_7;
  wire signed [63:0] c$case_alt_19;
  assign acc_1_1 = c$case_alt_19;

  assign x_16 = acc_3;

  assign y_7 = acc_4;

  assign c$case_alt_19 = (x_16 + y_7);



  wire signed [63:0] x_17;
  wire signed [63:0] y_8;
  wire signed [63:0] c$case_alt_20;
  assign acc_1_2 = c$case_alt_20;

  assign x_17 = acc_5;

  assign y_8 = acc_6;

  assign c$case_alt_20 = (x_17 + y_8);



  wire signed [63:0] x_18;
  wire signed [63:0] y_9;
  wire signed [63:0] c$case_alt_21;
  assign acc_1_3 = c$case_alt_21;

  assign x_18 = acc_7;

  assign y_9 = acc_8;

  assign c$case_alt_21 = (x_18 + y_9);



  wire signed [63:0] x_19;
  wire signed [63:0] y_10;
  wire signed [63:0] c$case_alt_22;
  assign acc_1_4 = c$case_alt_22;

  assign x_19 = acc_9;

  assign y_10 = acc_10;

  assign c$case_alt_22 = (x_19 + y_10);



  wire signed [63:0] x_20;
  wire signed [63:0] y_11;
  wire signed [63:0] c$case_alt_23;
  assign acc_2_0 = c$case_alt_23;

  assign x_20 = acc_1_0;

  assign y_11 = acc_1_1;

  assign c$case_alt_23 = (x_20 + y_11);



  wire signed [63:0] x_21;
  wire signed [63:0] y_12;
  wire signed [63:0] c$case_alt_24;
  assign acc_2_1 = c$case_alt_24;

  assign x_21 = acc_1_2;

  assign y_12 = acc_1_3;

  assign c$case_alt_24 = (x_21 + y_12);



  wire signed [63:0] x_22;
  wire signed [63:0] y_13;
  wire signed [63:0] c$case_alt_25;
  assign acc_3_0 = c$case_alt_25;

  assign x_22 = acc_2_0;

  assign y_13 = acc_2_1;

  assign c$case_alt_25 = (x_22 + y_13);



  wire signed [63:0] x_23;
  wire signed [63:0] y_14;
  wire signed [63:0] c$case_alt_26;
  assign acc_4_0 = c$case_alt_26;

  assign x_23 = acc_3_0;

  assign y_14 = acc_1_4;

  assign c$case_alt_26 = (x_23 + y_14);





  assign result_24 = b ? t_2 : result_25;

  // register begin
  always @(posedge clk or  posedge  rst) begin : result_25_register
    if ( rst) begin
      result_25 <= {8'd13,   64'sd0};
    end else if (en) begin
      result_25 <= result_24;
    end
  end
  // register end

  assign b = result_42;

  assign t_2 = {result_33[15:8],   c$t_app_arg};

  assign x_projection_19 = $signed(win[704-1 -: 64]);

  assign x_5 = x_projection_19;

  assign win = window[703:0];

  // register begin
  always @(posedge clk or  posedge  rst) begin : window_register
    if ( rst) begin
      window <= {8'd13,   {64'sd0,   64'sd0,   64'sd0,   64'sd0,   64'sd0,   64'sd0,   64'sd0,
             64'sd0,   64'sd0,   64'sd0,   64'sd0}};
    end else if (en) begin
      window <= result_26;
    end
  end
  // register end

  assign result_26 = b_0 ? t_3 : window;

  assign t_3 = {result_30[7:0],   result_27};

  assign result_27 = result_31 ? c$t_case_alt : c$t_case_alt_0;

  assign c$t_case_alt_sel_alt_t_0 = ({64'sd0,lastBucketUpdated});

  assign c$t_case_alt_sel_alt_f_0 = ({64'sd0,win});

  assign c$t_case_alt = c$b_case_alt ? c$t_case_alt_sel_alt_t_0[767:64] : c$t_case_alt_sel_alt_f_0[767:64];

  assign c$t_case_alt_0 = c$b_case_alt ? lastBucketUpdated : win;

  // vector replace begin
  genvar i_21;
  generate
  for (i_21=0;i_21<11;i_21=i_21+1) begin : vector_replace
    assign lastBucketUpdated[(10-i_21)*64+:64] = (64'sd0) == i_21 ? ((x_5 + 64'sd1)) : win[(10-i_21)*64+:64];
  end
  endgenerate
  // vector replace end

  assign b_0 = c$b_case_alt | result_31;

  assign c$b_case_alt = x0_7 & x1_7;

  assign x1_7 = result_32[0:0];

  assign x0_7 = result_32[1:1];

  assign result_28 = (x0_8 & x1_8) ? t_4 : result_29;

  assign x0_8 = result_59[1:1];

  assign x1_8 = result_59[0:0];

  // register begin
  always @(posedge clk or  posedge  rst) begin : result_29_register
    if ( rst) begin
      result_29 <= {8'd13,   1'b0};
    end else if (en) begin
      result_29 <= result_28;
    end
  end
  // register end

  assign t_4 = {result_44[23:16],
                (c$out4_case_alt < (64'sd5)) | (c$out4_case_alt > (64'sd10))};

  assign c$out4_case_alt = (tag_0 == tagToMatch) ? dta_0 : (64'sd0);

  assign dta_0 = $signed(result_23[63:0]);

  assign tag_0 = result_23[71:64];

  assign tagToMatch = result_44[31:24];

  wire [71:0] iterateI_ho1_0_arg0_0;
  wire [71:0] iterateI_ho1_0_res_5;
  wire [71:0] iterateI_ho1_1_res_5;
  wire [71:0] iterateI_ho1_2_res_5;
  wire [71:0] iterateI_ho1_3_res_5;
  assign iterateI_ho1_0_arg0_0 = {t_13,   t_12,
                                  t_11,   t_10,   t_9,   t_8,   t_7,   t_6,
                                  t_7};

  reg [71:0] c$bb_res_res_41 = {8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_41_delay
    if (en) begin
      c$bb_res_res_41 <= iterateI_ho1_0_arg0_0;
    end
  end
  // delay end

  assign iterateI_ho1_0_res_5 = c$bb_res_res_41;



  reg [71:0] c$bb_res_res_42 = {8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_42_delay
    if (en) begin
      c$bb_res_res_42 <= iterateI_ho1_0_res_5;
    end
  end
  // delay end

  assign iterateI_ho1_1_res_5 = c$bb_res_res_42;



  reg [71:0] c$bb_res_res_43 = {8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_43_delay
    if (en) begin
      c$bb_res_res_43 <= iterateI_ho1_1_res_5;
    end
  end
  // delay end

  assign iterateI_ho1_2_res_5 = c$bb_res_res_43;



  reg [71:0] c$bb_res_res_44 = {8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_44_delay
    if (en) begin
      c$bb_res_res_44 <= iterateI_ho1_2_res_5;
    end
  end
  // delay end

  assign iterateI_ho1_3_res_5 = c$bb_res_res_44;



  assign c$app_arg_6 = {{t_13,   t_12,   t_11,
                         t_10,   t_9,   t_8,   t_7,   t_6,   t_7},
                        iterateI_ho1_0_res_5,   iterateI_ho1_1_res_5,
                        iterateI_ho1_2_res_5,   iterateI_ho1_3_res_5};



  assign result_30 = c$app_arg_6[72-1:0];

  wire  iterateI_ho1_0_res_6;
  wire  iterateI_ho1_1_res_6;
  wire  iterateI_ho1_2_res_6;
  wire  iterateI_ho1_3_res_6;
  wire  iterateI_ho1_4_res_5;
  reg  c$bb_res_res_45 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_45_delay
    if (en) begin
      c$bb_res_res_45 <= slides;
    end
  end
  // delay end

  assign iterateI_ho1_0_res_6 = c$bb_res_res_45;



  reg  c$bb_res_res_46 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_46_delay
    if (en) begin
      c$bb_res_res_46 <= iterateI_ho1_0_res_6;
    end
  end
  // delay end

  assign iterateI_ho1_1_res_6 = c$bb_res_res_46;



  reg  c$bb_res_res_47 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_47_delay
    if (en) begin
      c$bb_res_res_47 <= iterateI_ho1_1_res_6;
    end
  end
  // delay end

  assign iterateI_ho1_2_res_6 = c$bb_res_res_47;



  reg  c$bb_res_res_48 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_48_delay
    if (en) begin
      c$bb_res_res_48 <= iterateI_ho1_2_res_6;
    end
  end
  // delay end

  assign iterateI_ho1_3_res_6 = c$bb_res_res_48;



  reg  c$bb_res_res_49 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_49_delay
    if (en) begin
      c$bb_res_res_49 <= iterateI_ho1_3_res_6;
    end
  end
  // delay end

  assign iterateI_ho1_4_res_5 = c$bb_res_res_49;



  assign c$app_arg_7 = {slides,
                        iterateI_ho1_0_res_6,   iterateI_ho1_1_res_6,
                        iterateI_ho1_2_res_6,   iterateI_ho1_3_res_6,
                        iterateI_ho1_4_res_5};



  assign result_31 = c$app_arg_7[1-1:0];

  assign slides = result_74[13:13];

  wire [1:0] iterateI_ho1_0_res_7;
  wire [1:0] iterateI_ho1_1_res_7;
  wire [1:0] iterateI_ho1_2_res_7;
  wire [1:0] iterateI_ho1_3_res_7;
  wire [1:0] iterateI_ho1_4_res_6;
  reg [1:0] c$bb_res_res_50 = {1'b0,   1'b0};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_50_delay
    if (en) begin
      c$bb_res_res_50 <= pOut4;
    end
  end
  // delay end

  assign iterateI_ho1_0_res_7 = c$bb_res_res_50;



  reg [1:0] c$bb_res_res_51 = {1'b0,   1'b0};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_51_delay
    if (en) begin
      c$bb_res_res_51 <= iterateI_ho1_0_res_7;
    end
  end
  // delay end

  assign iterateI_ho1_1_res_7 = c$bb_res_res_51;



  reg [1:0] c$bb_res_res_52 = {1'b0,   1'b0};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_52_delay
    if (en) begin
      c$bb_res_res_52 <= iterateI_ho1_1_res_7;
    end
  end
  // delay end

  assign iterateI_ho1_2_res_7 = c$bb_res_res_52;



  reg [1:0] c$bb_res_res_53 = {1'b0,   1'b0};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_53_delay
    if (en) begin
      c$bb_res_res_53 <= iterateI_ho1_2_res_7;
    end
  end
  // delay end

  assign iterateI_ho1_3_res_7 = c$bb_res_res_53;



  reg [1:0] c$bb_res_res_54 = {1'b0,   1'b0};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_54_delay
    if (en) begin
      c$bb_res_res_54 <= iterateI_ho1_3_res_7;
    end
  end
  // delay end

  assign iterateI_ho1_4_res_6 = c$bb_res_res_54;



  assign c$app_arg_8 = {pOut4,
                        iterateI_ho1_0_res_7,   iterateI_ho1_1_res_7,
                        iterateI_ho1_2_res_7,   iterateI_ho1_3_res_7,
                        iterateI_ho1_4_res_6};



  assign result_32 = c$app_arg_8[2-1:0];

  wire [71:0] iterateI_ho1_0_arg0_1;
  wire [71:0] iterateI_ho1_0_res_8;
  wire [71:0] iterateI_ho1_1_res_8;
  wire [71:0] iterateI_ho1_2_res_8;
  wire [71:0] iterateI_ho1_3_res_8;
  wire [71:0] iterateI_ho1_4_res_7;
  assign iterateI_ho1_0_arg0_1 = {t_13,   t_12,
                                  t_11,   t_10,   t_9,   t_8,   t_7,   t_6,
                                  t_7};

  reg [71:0] c$bb_res_res_55 = {8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_55_delay
    if (en) begin
      c$bb_res_res_55 <= iterateI_ho1_0_arg0_1;
    end
  end
  // delay end

  assign iterateI_ho1_0_res_8 = c$bb_res_res_55;



  reg [71:0] c$bb_res_res_56 = {8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_56_delay
    if (en) begin
      c$bb_res_res_56 <= iterateI_ho1_0_res_8;
    end
  end
  // delay end

  assign iterateI_ho1_1_res_8 = c$bb_res_res_56;



  reg [71:0] c$bb_res_res_57 = {8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_57_delay
    if (en) begin
      c$bb_res_res_57 <= iterateI_ho1_1_res_8;
    end
  end
  // delay end

  assign iterateI_ho1_2_res_8 = c$bb_res_res_57;



  reg [71:0] c$bb_res_res_58 = {8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_58_delay
    if (en) begin
      c$bb_res_res_58 <= iterateI_ho1_2_res_8;
    end
  end
  // delay end

  assign iterateI_ho1_3_res_8 = c$bb_res_res_58;



  reg [71:0] c$bb_res_res_59 = {8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_59_delay
    if (en) begin
      c$bb_res_res_59 <= iterateI_ho1_3_res_8;
    end
  end
  // delay end

  assign iterateI_ho1_4_res_7 = c$bb_res_res_59;



  assign c$app_arg_9 = {{t_13,   t_12,   t_11,
                         t_10,   t_9,   t_8,   t_7,   t_6,   t_7},
                        iterateI_ho1_0_res_8,   iterateI_ho1_1_res_8,
                        iterateI_ho1_2_res_8,   iterateI_ho1_3_res_8,
                        iterateI_ho1_4_res_7};



  assign result_33 = c$app_arg_9[72-1:0];

  assign result_34 = (tag_1 > 8'd1) ? c$app_arg_10 : (c$app_arg_10 + 8'd12);

  assign c$app_arg_10 = tag_1 - 8'd1;

  assign tag_1 = result_46[31:24];

  assign c$vec2_4 = (ws_4[4-1 : 0]);

  // zipWith start
  genvar i_22;
  generate
  for (i_22 = 0; i_22 < 2; i_22 = i_22 + 1) begin : zipWith_4
    wire [71:0] zipWith_in1_4;
    assign zipWith_in1_4 = result_37[i_22*72+:72];
    wire [1:0] zipWith_in2_4;
    assign zipWith_in2_4 = c$vec2_4[i_22*2+:2];
    wire [73:0] c$n_4;
    assign c$n_4 = {zipWith_in1_4,   zipWith_in2_4};


    assign c$ws_app_arg_9[i_22*74+:74] = c$n_4;
  end
  endgenerate
  // zipWith end

  // imap begin
  genvar i_23;
  generate
  for (i_23=0; i_23 < 2; i_23 = i_23 + 1) begin : imap_4
    wire [1-1:0] map_index_4;
    wire [73:0] map_in_4;
    assign map_in_4 = c$ws_app_arg_9[i_23*74+:74];
    wire [1:0] map_out_4;

    assign map_index_4 = 1'd1 - i_23[0+:1];
    wire [1:0] c$case_alt_27;
    // spec.hs:349:1-63
    wire [7:0] t_20;
    // spec.hs:349:1-63
    wire [71:0] x_24;
    assign map_out_4 = c$case_alt_27;

    assign c$case_alt_27 = (t_20 == result_45[55:48]) ? {1'b1,map_index_4} : map_in_4[1:0];

    assign t_20 = x_24[71:64];

    assign x_24 = map_in_4[73:2];


    assign c$ws_app_arg_10[i_23*2+:2] = map_out_4;
  end
  endgenerate
  // imap end

  // index begin
  wire [71:0] vecArray_4 [0:2-1];
  genvar i_24;
  generate
  for (i_24=0; i_24 < 2; i_24=i_24+1) begin : mk_array_4
    assign vecArray_4[(2-1)-i_24] = result_37[i_24*72+:72];
  end
  endgenerate
  assign c$case_scrut_9 = vecArray_4[($unsigned({{(64-1) {1'b0}},i_4}))];
  // index end

  assign c$case_scrut_10 = ws_4[6-1 -: 2];

  assign result_35 = c$case_scrut_10[1:1] ? $signed(c$case_scrut_9[63:0]) : (64'sd0);

  assign i_4 = c$case_scrut_10[0:0];

  assign ws_4 = {c$ws_app_arg_10,{1'b0,1'bx}};

  assign result_36 = (x0_9 & x1_9) ? t_5 : result_37;

  assign x0_9 = result_63[1:1];

  assign x1_9 = result_63[0:0];

  // register begin
  always @(posedge clk or  posedge  rst) begin : result_37_register
    if ( rst) begin
      result_37 <= {{8'd13,   64'sd0},   {8'd13,   64'sd0}};
    end else if (en) begin
      result_37 <= result_36;
    end
  end
  // register end

  assign t_projection_7 = ({result_37,{result_46[55:48],   (x_6 + y_4)}});

  assign t_5 = t_projection_7[143:0];

  assign x_6 = c$out0_case_alt_0;

  assign y_4 = c$out0_case_alt;

  assign c$out0_case_alt = (tag_2 == tagToMatch_0) ? dta_1 : (64'sd0);

  assign dta_1 = $signed(result_39[63:0]);

  assign tag_2 = result_39[71:64];

  assign tagToMatch_0 = result_46[63:56];

  assign c$out0_case_alt_0 = (tag_3 == tagToMatch_1) ? dta_2 : (64'sd0);

  assign dta_2 = $signed(result_41[63:0]);

  assign tagToMatch_1 = result_46[71:64];

  assign tag_3 = result_41[71:64];

  assign result_38 = b_1 ? {t_12,
                            c$input1Win_app_arg} : result_39;

  // register begin
  always @(posedge clk or  posedge  rst) begin : result_39_register
    if ( rst) begin
      result_39 <= {8'd13,   64'sd0};
    end else if (en) begin
      result_39 <= result_38;
    end
  end
  // register end

  assign b_1 = result_64;

  // delay begin
  always @(posedge clk) begin : c$input1Win_app_arg_delay
    if (en) begin
      c$input1Win_app_arg <= $signed(inputs_2[64:1]);
    end
  end
  // delay end

  assign result_40 = b_2 ? {t_13,
                            c$input0Win_app_arg} : result_41;

  // register begin
  always @(posedge clk or  posedge  rst) begin : result_41_register
    if ( rst) begin
      result_41 <= {8'd13,   64'sd0};
    end else if (en) begin
      result_41 <= result_40;
    end
  end
  // register end

  assign b_2 = result_67;

  // delay begin
  always @(posedge clk) begin : c$input0Win_app_arg_delay
    if (en) begin
      c$input0Win_app_arg <= $signed(inputs_2[129:66]);
    end
  end
  // delay end

  assign inputs_2 = result_74[143:14];

  wire  iterateI_ho1_0_res_9;
  wire  iterateI_ho1_1_res_9;
  wire  iterateI_ho1_2_res_9;
  wire  iterateI_ho1_3_res_9;
  wire  iterateI_ho1_4_res_8;
  wire  iterateI_ho1_5_res_5;
  reg  c$bb_res_res_60 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_60_delay
    if (en) begin
      c$bb_res_res_60 <= pOut5;
    end
  end
  // delay end

  assign iterateI_ho1_0_res_9 = c$bb_res_res_60;



  reg  c$bb_res_res_61 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_61_delay
    if (en) begin
      c$bb_res_res_61 <= iterateI_ho1_0_res_9;
    end
  end
  // delay end

  assign iterateI_ho1_1_res_9 = c$bb_res_res_61;



  reg  c$bb_res_res_62 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_62_delay
    if (en) begin
      c$bb_res_res_62 <= iterateI_ho1_1_res_9;
    end
  end
  // delay end

  assign iterateI_ho1_2_res_9 = c$bb_res_res_62;



  reg  c$bb_res_res_63 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_63_delay
    if (en) begin
      c$bb_res_res_63 <= iterateI_ho1_2_res_9;
    end
  end
  // delay end

  assign iterateI_ho1_3_res_9 = c$bb_res_res_63;



  reg  c$bb_res_res_64 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_64_delay
    if (en) begin
      c$bb_res_res_64 <= iterateI_ho1_3_res_9;
    end
  end
  // delay end

  assign iterateI_ho1_4_res_8 = c$bb_res_res_64;



  reg  c$bb_res_res_65 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_65_delay
    if (en) begin
      c$bb_res_res_65 <= iterateI_ho1_4_res_8;
    end
  end
  // delay end

  assign iterateI_ho1_5_res_5 = c$bb_res_res_65;



  assign c$app_arg_11 = {pOut5,
                         iterateI_ho1_0_res_9,   iterateI_ho1_1_res_9,
                         iterateI_ho1_2_res_9,   iterateI_ho1_3_res_9,
                         iterateI_ho1_4_res_8,   iterateI_ho1_5_res_5};



  assign result_42 = c$app_arg_11[1-1:0];

  wire [71:0] iterateI_ho1_0_arg0_2;
  wire [71:0] iterateI_ho1_0_res_10;
  wire [71:0] iterateI_ho1_1_res_10;
  wire [71:0] iterateI_ho1_2_res_10;
  wire [71:0] iterateI_ho1_3_res_10;
  wire [71:0] iterateI_ho1_4_res_9;
  wire [71:0] iterateI_ho1_5_res_6;
  assign iterateI_ho1_0_arg0_2 = {t_13,   t_12,
                                  t_11,   t_10,   t_9,   t_8,   t_7,   t_6,
                                  t_7};

  reg [71:0] c$bb_res_res_66 = {8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_66_delay
    if (en) begin
      c$bb_res_res_66 <= iterateI_ho1_0_arg0_2;
    end
  end
  // delay end

  assign iterateI_ho1_0_res_10 = c$bb_res_res_66;



  reg [71:0] c$bb_res_res_67 = {8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_67_delay
    if (en) begin
      c$bb_res_res_67 <= iterateI_ho1_0_res_10;
    end
  end
  // delay end

  assign iterateI_ho1_1_res_10 = c$bb_res_res_67;



  reg [71:0] c$bb_res_res_68 = {8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_68_delay
    if (en) begin
      c$bb_res_res_68 <= iterateI_ho1_1_res_10;
    end
  end
  // delay end

  assign iterateI_ho1_2_res_10 = c$bb_res_res_68;



  reg [71:0] c$bb_res_res_69 = {8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_69_delay
    if (en) begin
      c$bb_res_res_69 <= iterateI_ho1_2_res_10;
    end
  end
  // delay end

  assign iterateI_ho1_3_res_10 = c$bb_res_res_69;



  reg [71:0] c$bb_res_res_70 = {8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_70_delay
    if (en) begin
      c$bb_res_res_70 <= iterateI_ho1_3_res_10;
    end
  end
  // delay end

  assign iterateI_ho1_4_res_9 = c$bb_res_res_70;



  reg [71:0] c$bb_res_res_71 = {8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_71_delay
    if (en) begin
      c$bb_res_res_71 <= iterateI_ho1_4_res_9;
    end
  end
  // delay end

  assign iterateI_ho1_5_res_6 = c$bb_res_res_71;



  assign c$app_arg_12 = {{t_13,   t_12,   t_11,
                          t_10,   t_9,   t_8,   t_7,   t_6,   t_7},
                         iterateI_ho1_0_res_10,
                         iterateI_ho1_1_res_10,
                         iterateI_ho1_2_res_10,
                         iterateI_ho1_3_res_10,   iterateI_ho1_4_res_9,
                         iterateI_ho1_5_res_6};



  assign result_43 = c$app_arg_12[72-1:0];

  wire [71:0] iterateI_ho1_0_arg0_3;
  wire [71:0] iterateI_ho1_0_res_11;
  wire [71:0] iterateI_ho1_1_res_11;
  wire [71:0] iterateI_ho1_2_res_11;
  assign iterateI_ho1_0_arg0_3 = {t_13,   t_12,
                                  t_11,   t_10,   t_9,   t_8,   t_7,   t_6,
                                  t_7};

  reg [71:0] c$bb_res_res_72 = {8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_72_delay
    if (en) begin
      c$bb_res_res_72 <= iterateI_ho1_0_arg0_3;
    end
  end
  // delay end

  assign iterateI_ho1_0_res_11 = c$bb_res_res_72;



  reg [71:0] c$bb_res_res_73 = {8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_73_delay
    if (en) begin
      c$bb_res_res_73 <= iterateI_ho1_0_res_11;
    end
  end
  // delay end

  assign iterateI_ho1_1_res_11 = c$bb_res_res_73;



  reg [71:0] c$bb_res_res_74 = {8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_74_delay
    if (en) begin
      c$bb_res_res_74 <= iterateI_ho1_1_res_11;
    end
  end
  // delay end

  assign iterateI_ho1_2_res_11 = c$bb_res_res_74;



  assign c$app_arg_13 = {{t_13,   t_12,   t_11,
                          t_10,   t_9,   t_8,   t_7,   t_6,   t_7},
                         iterateI_ho1_0_res_11,
                         iterateI_ho1_1_res_11,
                         iterateI_ho1_2_res_11};



  assign result_44 = c$app_arg_13[72-1:0];

  wire [71:0] iterateI_ho1_0_arg0_4;
  wire [71:0] iterateI_ho1_0_res_12;
  wire [71:0] iterateI_ho1_1_res_12;
  assign iterateI_ho1_0_arg0_4 = {t_13,   t_12,
                                  t_11,   t_10,   t_9,   t_8,   t_7,   t_6,
                                  t_7};

  reg [71:0] c$bb_res_res_75 = {8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_75_delay
    if (en) begin
      c$bb_res_res_75 <= iterateI_ho1_0_arg0_4;
    end
  end
  // delay end

  assign iterateI_ho1_0_res_12 = c$bb_res_res_75;



  reg [71:0] c$bb_res_res_76 = {8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_76_delay
    if (en) begin
      c$bb_res_res_76 <= iterateI_ho1_0_res_12;
    end
  end
  // delay end

  assign iterateI_ho1_1_res_12 = c$bb_res_res_76;



  assign c$app_arg_14 = {{t_13,   t_12,   t_11,
                          t_10,   t_9,   t_8,   t_7,   t_6,   t_7},
                         iterateI_ho1_0_res_12,
                         iterateI_ho1_1_res_12};



  assign result_45 = c$app_arg_14[72-1:0];

  wire [71:0] iterateI_ho1_0_arg0_5;
  wire [71:0] iterateI_ho1_0_res_13;
  assign iterateI_ho1_0_arg0_5 = {t_13,   t_12,
                                  t_11,   t_10,   t_9,   t_8,   t_7,   t_6,
                                  t_7};

  reg [71:0] c$bb_res_res_77 = {8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13,   8'd13};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_77_delay
    if (en) begin
      c$bb_res_res_77 <= iterateI_ho1_0_arg0_5;
    end
  end
  // delay end

  assign iterateI_ho1_0_res_13 = c$bb_res_res_77;



  assign c$app_arg_15 = {{t_13,   t_12,   t_11,
                          t_10,   t_9,   t_8,   t_7,   t_6,   t_7},
                         iterateI_ho1_0_res_13};



  assign result_46 = c$app_arg_15[72-1:0];

  // register begin
  always @(posedge clk or  posedge  rst) begin : t_6_register
    if ( rst) begin
      t_6 <= 8'd1;
    end else if (en) begin
      t_6 <= result_47;
    end
  end
  // register end

  assign result_47 = pOut5 ? result_48 : t_6;

  assign b_3 = t_6 == 8'd12;

  assign f1 = t_6 + 8'd1;

  assign result_48 = b_3 ? 8'd1 : f1;

  assign pOut5 = pacings[0:0];

  // register begin
  always @(posedge clk or  posedge  rst) begin : t_7_register
    if ( rst) begin
      t_7 <= 8'd1;
    end else if (en) begin
      t_7 <= result_49;
    end
  end
  // register end

  assign result_49 = c$ds8_case_alt ? result_50 : t_7;

  assign b_4 = t_7 == 8'd12;

  assign f1_0 = t_7 + 8'd1;

  assign result_50 = b_4 ? 8'd1 : f1_0;

  assign c$ds8_case_alt = x0_10 & x1_10;

  assign x1_10 = pOut4[0:0];

  assign x0_10 = pOut4[1:1];

  // register begin
  always @(posedge clk or  posedge  rst) begin : t_8_register
    if ( rst) begin
      t_8 <= 8'd1;
    end else if (en) begin
      t_8 <= result_51;
    end
  end
  // register end

  assign result_51 = c$ds7_case_alt ? result_52 : t_8;

  assign b_5 = t_8 == 8'd12;

  assign f1_1 = t_8 + 8'd1;

  assign result_52 = b_5 ? 8'd1 : f1_1;

  assign c$ds7_case_alt = x0_11 & x1_11;

  assign x1_11 = pOut3[0:0];

  assign x0_11 = pOut3[1:1];

  // register begin
  always @(posedge clk or  posedge  rst) begin : t_9_register
    if ( rst) begin
      t_9 <= 8'd1;
    end else if (en) begin
      t_9 <= result_53;
    end
  end
  // register end

  assign result_53 = c$ds6_case_alt ? result_54 : t_9;

  assign b_6 = t_9 == 8'd12;

  assign f1_2 = t_9 + 8'd1;

  assign result_54 = b_6 ? 8'd1 : f1_2;

  assign c$ds6_case_alt = x0_12 & x1_12;

  assign x1_12 = pOut2[0:0];

  assign x0_12 = pOut2[1:1];

  // register begin
  always @(posedge clk or  posedge  rst) begin : t_10_register
    if ( rst) begin
      t_10 <= 8'd1;
    end else if (en) begin
      t_10 <= result_55;
    end
  end
  // register end

  assign result_55 = c$ds5_case_alt ? result_56 : t_10;

  assign b_7 = t_10 == 8'd12;

  assign f1_3 = t_10 + 8'd1;

  assign result_56 = b_7 ? 8'd1 : f1_3;

  assign c$ds5_case_alt = x0_13 & x1_13;

  assign x1_13 = pOut1[0:0];

  assign x0_13 = pOut1[1:1];

  // register begin
  always @(posedge clk or  posedge  rst) begin : t_11_register
    if ( rst) begin
      t_11 <= 8'd1;
    end else if (en) begin
      t_11 <= result_57;
    end
  end
  // register end

  assign result_57 = c$ds4_case_alt ? result_58 : t_11;

  assign b_8 = t_11 == 8'd12;

  assign f1_4 = t_11 + 8'd1;

  assign result_58 = b_8 ? 8'd1 : f1_4;

  assign c$ds4_case_alt = x0_14 & x1_14;

  assign x1_14 = pOut0[0:0];

  assign x0_14 = pOut0[1:1];

  wire [1:0] iterateI_ho1_0_res_14;
  wire [1:0] iterateI_ho1_1_res_13;
  wire [1:0] iterateI_ho1_2_res_12;
  wire [1:0] iterateI_ho1_3_res_11;
  reg [1:0] c$bb_res_res_78 = {1'b0,   1'b0};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_78_delay
    if (en) begin
      c$bb_res_res_78 <= pOut4;
    end
  end
  // delay end

  assign iterateI_ho1_0_res_14 = c$bb_res_res_78;



  reg [1:0] c$bb_res_res_79 = {1'b0,   1'b0};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_79_delay
    if (en) begin
      c$bb_res_res_79 <= iterateI_ho1_0_res_14;
    end
  end
  // delay end

  assign iterateI_ho1_1_res_13 = c$bb_res_res_79;



  reg [1:0] c$bb_res_res_80 = {1'b0,   1'b0};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_80_delay
    if (en) begin
      c$bb_res_res_80 <= iterateI_ho1_1_res_13;
    end
  end
  // delay end

  assign iterateI_ho1_2_res_12 = c$bb_res_res_80;



  reg [1:0] c$bb_res_res_81 = {1'b0,   1'b0};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_81_delay
    if (en) begin
      c$bb_res_res_81 <= iterateI_ho1_2_res_12;
    end
  end
  // delay end

  assign iterateI_ho1_3_res_11 = c$bb_res_res_81;



  assign c$app_arg_16 = {pOut4,
                         iterateI_ho1_0_res_14,
                         iterateI_ho1_1_res_13,
                         iterateI_ho1_2_res_12,
                         iterateI_ho1_3_res_11};



  assign result_59 = c$app_arg_16[2-1:0];

  assign pOut4 = pacings[2:1];

  wire [1:0] iterateI_ho1_0_res_15;
  wire [1:0] iterateI_ho1_1_res_14;
  wire [1:0] iterateI_ho1_2_res_13;
  reg [1:0] c$bb_res_res_82 = {1'b0,   1'b0};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_82_delay
    if (en) begin
      c$bb_res_res_82 <= pOut3;
    end
  end
  // delay end

  assign iterateI_ho1_0_res_15 = c$bb_res_res_82;



  reg [1:0] c$bb_res_res_83 = {1'b0,   1'b0};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_83_delay
    if (en) begin
      c$bb_res_res_83 <= iterateI_ho1_0_res_15;
    end
  end
  // delay end

  assign iterateI_ho1_1_res_14 = c$bb_res_res_83;



  reg [1:0] c$bb_res_res_84 = {1'b0,   1'b0};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_84_delay
    if (en) begin
      c$bb_res_res_84 <= iterateI_ho1_1_res_14;
    end
  end
  // delay end

  assign iterateI_ho1_2_res_13 = c$bb_res_res_84;



  assign c$app_arg_17 = {pOut3,
                         iterateI_ho1_0_res_15,
                         iterateI_ho1_1_res_14,
                         iterateI_ho1_2_res_13};



  assign result_60 = c$app_arg_17[2-1:0];

  assign pOut3 = pacings[4:3];

  wire [1:0] iterateI_ho1_0_res_16;
  wire [1:0] iterateI_ho1_1_res_15;
  reg [1:0] c$bb_res_res_85 = {1'b0,   1'b0};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_85_delay
    if (en) begin
      c$bb_res_res_85 <= pOut2;
    end
  end
  // delay end

  assign iterateI_ho1_0_res_16 = c$bb_res_res_85;



  reg [1:0] c$bb_res_res_86 = {1'b0,   1'b0};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_86_delay
    if (en) begin
      c$bb_res_res_86 <= iterateI_ho1_0_res_16;
    end
  end
  // delay end

  assign iterateI_ho1_1_res_15 = c$bb_res_res_86;



  assign c$app_arg_18 = {pOut2,
                         iterateI_ho1_0_res_16,
                         iterateI_ho1_1_res_15};



  assign result_61 = c$app_arg_18[2-1:0];

  assign pOut2 = pacings[6:5];

  wire [1:0] iterateI_ho1_0_res_17;
  wire [1:0] iterateI_ho1_1_res_16;
  reg [1:0] c$bb_res_res_87 = {1'b0,   1'b0};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_87_delay
    if (en) begin
      c$bb_res_res_87 <= pOut1;
    end
  end
  // delay end

  assign iterateI_ho1_0_res_17 = c$bb_res_res_87;



  reg [1:0] c$bb_res_res_88 = {1'b0,   1'b0};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_88_delay
    if (en) begin
      c$bb_res_res_88 <= iterateI_ho1_0_res_17;
    end
  end
  // delay end

  assign iterateI_ho1_1_res_16 = c$bb_res_res_88;



  assign c$app_arg_19 = {pOut1,
                         iterateI_ho1_0_res_17,
                         iterateI_ho1_1_res_16};



  assign result_62 = c$app_arg_19[2-1:0];

  assign pOut1 = pacings[8:7];

  wire [1:0] iterateI_ho1_0_res_18;
  wire [1:0] iterateI_ho1_1_res_17;
  reg [1:0] c$bb_res_res_89 = {1'b0,   1'b0};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_89_delay
    if (en) begin
      c$bb_res_res_89 <= pOut0;
    end
  end
  // delay end

  assign iterateI_ho1_0_res_18 = c$bb_res_res_89;



  reg [1:0] c$bb_res_res_90 = {1'b0,   1'b0};
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_90_delay
    if (en) begin
      c$bb_res_res_90 <= iterateI_ho1_0_res_18;
    end
  end
  // delay end

  assign iterateI_ho1_1_res_17 = c$bb_res_res_90;



  assign c$app_arg_20 = {pOut0,
                         iterateI_ho1_0_res_18,
                         iterateI_ho1_1_res_17};



  assign result_63 = c$app_arg_20[2-1:0];

  assign pOut0 = pacings[10:9];

  wire  iterateI_ho1_0_res_19;
  reg  c$bb_res_res_91 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_91_delay
    if (en) begin
      c$bb_res_res_91 <= pIn1_0;
    end
  end
  // delay end

  assign iterateI_ho1_0_res_19 = c$bb_res_res_91;



  assign c$app_arg_21 = {pIn1_0,
                         iterateI_ho1_0_res_19};



  assign result_64 = c$app_arg_21[1-1:0];

  // register begin
  always @(posedge clk or  posedge  rst) begin : t_12_register
    if ( rst) begin
      t_12 <= 8'd1;
    end else if (en) begin
      t_12 <= result_65;
    end
  end
  // register end

  assign result_65 = pIn1_0 ? result_66 : t_12;

  assign b_9 = t_12 == 8'd12;

  assign f1_5 = t_12 + 8'd1;

  assign result_66 = b_9 ? 8'd1 : f1_5;

  assign pIn1_0 = pacings[11:11];

  wire  iterateI_ho1_0_res_20;
  reg  c$bb_res_res_92 = 1'b0;
  // delay begin
  always @(posedge clk) begin : c$bb_res_res_92_delay
    if (en) begin
      c$bb_res_res_92 <= pIn0_0;
    end
  end
  // delay end

  assign iterateI_ho1_0_res_20 = c$bb_res_res_92;



  assign c$app_arg_22 = {pIn0_0,
                         iterateI_ho1_0_res_20};



  assign result_67 = c$app_arg_22[1-1:0];

  // register begin
  always @(posedge clk or  posedge  rst) begin : t_13_register
    if ( rst) begin
      t_13 <= 8'd1;
    end else if (en) begin
      t_13 <= result_68;
    end
  end
  // register end

  assign result_68 = pIn0_0 ? result_69 : t_13;

  assign b_10 = t_13 == 8'd12;

  assign f1_6 = t_13 + 8'd1;

  assign result_69 = b_10 ? 8'd1 : f1_6;

  assign pIn0_0 = pacings[12:12];

  assign pacings = result_74[12:0];

  // register begin
  always @(posedge clk or  posedge  rst) begin : toWait_register
    if ( rst) begin
      toWait <= (64'sd0);
    end else if (en) begin
      toWait <= result_71;
    end
  end
  // register end

  assign result_70 = toWait == (64'sd0);

  assign result_71 = result_73 ? (64'sd3) : result_72;

  assign b_11 = toWait > (64'sd0);

  assign result_72 = b_11 ? ((x_7 - 64'sd1)) : toWait;

  assign x_7 = toWait;

  assign result_73 = b_12 ? 1'b1 : 1'b0;

  assign b_12 = result_70 & qPopValid;

  assign result_74 = {c$app_arg_25,
                      c$app_arg_24,   c$app_arg_23};

  // register begin
  always @(posedge clk or  posedge  rst) begin : c$app_arg_23_register
    if ( rst) begin
      c$app_arg_23 <= {{{64'sd0,   1'b0},   {64'sd0,   1'b0}},   1'b0,   {1'b0,   1'b0,   {1'b0,
                                                                       1'b0},
                                                      {1'b0,   1'b0},   {1'b0,
                                                                         1'b0},
                                                      {1'b0,   1'b0},   {1'b0,
                                                                         1'b0},
                                                      1'b0}};
    end else if (en) begin
      c$app_arg_23 <= c$case_alt;
    end
  end
  // register end

  assign c$case_alt = qPush ? c$case_alt_0 : c$case_alt_1;

  assign c$case_alt_0 = qPop ? c$case_alt_2 : {{{64'sd0,
                                                 1'b0},   {64'sd0,   1'b0}},   1'b0,   {1'b0,
                                                                                        1'b0,   {1'b0,   1'b0},
                                                                                        {1'b0,   1'b0},   {1'b0,
                                                                                                           1'b0},
                                                                                        {1'b0,   1'b0},   {1'b0,
                                                                                                           1'b0},
                                                                                        1'b0}};

  assign c$case_alt_1 = qPop ? c$case_alt_3 : {{{64'sd0,
                                                 1'b0},   {64'sd0,   1'b0}},   1'b0,   {1'b0,
                                                                                        1'b0,   {1'b0,   1'b0},
                                                                                        {1'b0,   1'b0},   {1'b0,
                                                                                                           1'b0},
                                                                                        {1'b0,   1'b0},   {1'b0,
                                                                                                           1'b0},
                                                                                        1'b0}};

  always @(*) begin
    case(x_8)
      64'sd0 : c$case_alt_2 = qData;
      default : c$case_alt_2 = c$case_alt_4;
    endcase
  end

  always @(*) begin
    case(x_8)
      64'sd0 : c$case_alt_3 = {{{64'sd0,   1'b0},
                                {64'sd0,   1'b0}},   1'b0,   {1'b0,   1'b0,
                                                              {1'b0,   1'b0},   {1'b0,   1'b0},   {1'b0,
                                                                                                   1'b0},   {1'b0,
                                                                                                             1'b0},
                                                              {1'b0,   1'b0},   1'b0}};
      default : c$case_alt_3 = c$case_alt_4;
    endcase
  end

  // index begin
  wire [143:0] vecArray_5 [0:5-1];
  genvar i_25;
  generate
  for (i_25=0; i_25 < 5; i_25=i_25+1) begin : mk_array_5
    assign vecArray_5[(5-1)-i_25] = buffer[i_25*144+:144];
  end
  endgenerate
  assign c$case_alt_4 = vecArray_5[c$cursor_case_alt_4];
  // index end

  // register begin
  always @(posedge clk or  posedge  rst) begin : c$app_arg_24_register
    if ( rst) begin
      c$app_arg_24 <= 1'b0;
    end else if (en) begin
      c$app_arg_24 <= c$case_alt_5;
    end
  end
  // register end

  assign c$case_alt_5 = qPush ? c$case_alt_6 : c$case_alt_7;

  assign c$case_alt_6 = qPop ? 1'b1 : c$case_alt_8;

  assign c$case_alt_7 = qPop ? c$case_alt_9 : c$case_alt_8;

  assign c$case_alt_8 = qPop ? ({1 {1'bx}}) : 1'b0;

  always @(*) begin
    case(x_8)
      64'sd0 : c$case_alt_9 = 1'b0;
      default : c$case_alt_9 = 1'b1;
    endcase
  end

  // register begin
  always @(posedge clk or  posedge  rst) begin : c$app_arg_25_register
    if ( rst) begin
      c$app_arg_25 <= 1'b0;
    end else if (en) begin
      c$app_arg_25 <= c$case_alt_10;
    end
  end
  // register end

  assign c$case_alt_10 = qPush ? c$case_alt_11 : 1'b0;

  assign c$case_alt_11 = qPop ? 1'b1 : c$case_alt_12;

  assign c$tte_rhs = (x_8 != 64'sd5) ? 64'sd1 : 64'sd0;

  assign c$case_alt_12 = c$tte_rhs[0];

  // register begin
  always @(posedge clk or  posedge  rst) begin : buffer_register
    if ( rst) begin
      buffer <= {{{{64'sd0,   1'b0},   {64'sd0,   1'b0}},   1'b0,   {1'b0,   1'b0,   {1'b0,
                                                                        1'b0},
                                                       {1'b0,   1'b0},   {1'b0,
                                                                          1'b0},
                                                       {1'b0,   1'b0},   {1'b0,
                                                                          1'b0},
                                                       1'b0}},   {{{64'sd0,
                                                                    1'b0},
                                                                   {64'sd0,
                                                                    1'b0}},
                                                                  1'b0,   {1'b0,
                                                                           1'b0,
                                                                           {1'b0,
                                                                            1'b0},
                                                                           {1'b0,
                                                                            1'b0},
                                                                           {1'b0,
                                                                            1'b0},
                                                                           {1'b0,
                                                                            1'b0},
                                                                           {1'b0,
                                                                            1'b0},
                                                                           1'b0}},
   {{{64'sd0,   1'b0},   {64'sd0,   1'b0}},   1'b0,   {1'b0,   1'b0,   {1'b0,
                                                                        1'b0},
                                                       {1'b0,   1'b0},   {1'b0,
                                                                          1'b0},
                                                       {1'b0,   1'b0},   {1'b0,
                                                                          1'b0},
                                                       1'b0}},   {{{64'sd0,
                                                                    1'b0},
                                                                   {64'sd0,
                                                                    1'b0}},
                                                                  1'b0,   {1'b0,
                                                                           1'b0,
                                                                           {1'b0,
                                                                            1'b0},
                                                                           {1'b0,
                                                                            1'b0},
                                                                           {1'b0,
                                                                            1'b0},
                                                                           {1'b0,
                                                                            1'b0},
                                                                           {1'b0,
                                                                            1'b0},
                                                                           1'b0}},
   {{{64'sd0,   1'b0},   {64'sd0,   1'b0}},   1'b0,   {1'b0,   1'b0,   {1'b0,
                                                                        1'b0},
                                                       {1'b0,   1'b0},   {1'b0,
                                                                          1'b0},
                                                       {1'b0,   1'b0},   {1'b0,
                                                                          1'b0},
                                                       1'b0}}};
    end else if (en) begin
      buffer <= c$buffer_case_alt;
    end
  end
  // register end

  assign c$buffer_case_alt = qPush ? c$buffer_case_alt_0 : buffer;

  assign c$buffer_case_alt_sel_alt_t_1 = ({qData,buffer});

  assign c$buffer_case_alt_0 = qPop ? c$buffer_case_alt_sel_alt_t_1[863:144] : c$buffer_case_alt_1;

  assign c$buffer_case_alt_sel_alt_f_2 = ({qData,buffer});

  assign c$buffer_case_alt_1 = c$cursor_case_scrut ? buffer : c$buffer_case_alt_sel_alt_f_2[863:144];

  assign qData = result_4[143:0];

  assign x_8 = cursor;

  // register begin
  always @(posedge clk or  posedge  rst) begin : cursor_register
    if ( rst) begin
      cursor <= (64'sd0);
    end else if (en) begin
      cursor <= c$cursor_case_alt;
    end
  end
  // register end

  assign c$cursor_case_alt = qPush ? c$cursor_case_alt_0 : c$cursor_case_alt_2;

  assign c$cursor_case_alt_0 = qPop ? cursor : c$cursor_case_alt_1;

  assign c$cursor_case_alt_1 = c$cursor_case_scrut ? cursor : ((x_8 + 64'sd1));

  assign c$tte_rhs_0 = (x_8 == 64'sd5) ? 64'sd1 : 64'sd0;

  assign c$cursor_case_scrut = c$tte_rhs_0[0];

  assign c$cursor_case_alt_2 = qPop ? c$cursor_case_alt_3 : cursor;

  always @(*) begin
    case(x_8)
      64'sd0 : c$cursor_case_alt_3 = 64'sd0;
      default : c$cursor_case_alt_3 = c$cursor_case_alt_4;
    endcase
  end

  assign c$cursor_case_alt_4 = (x_8 - 64'sd1);

  assign result = {result_5[335:9],   {qPush,
                                       qPop,   result_74[145:145],   qPopValid,
                                       result_5[8:8],   result_5[7:0]}};

  assign qPopValid = result_74[144:144];

  assign qPush = result_4[144:144];

  assign qPop = result_5[336:336];

  assign result_0 = result[339:13];

  assign result_1 = result[12:0];

  assign result_0_0 = result_0[326:262];

  assign result_0_1 = result_0[261:197];

  assign result_0_2 = result_0[196:132];

  assign result_0_3 = result_0[131:67];

  assign result_0_4 = result_0[66:65];

  assign result_0_5 = result_0[64:0];

  assign result_0_0_0 = $signed(result_0_0[64:1]);

  assign result_0_0_1 = result_0_0[0:0];

  assign result_0_1_0 = $signed(result_0_1[64:1]);

  assign result_0_1_1 = result_0_1[0:0];

  assign result_0_2_0 = $signed(result_0_2[64:1]);

  assign result_0_2_1 = result_0_2[0:0];

  assign result_0_3_0 = $signed(result_0_3[64:1]);

  assign result_0_3_1 = result_0_3[0:0];

  assign result_0_4_0 = result_0_4[1:1];

  assign result_0_4_1 = result_0_4[0:0];

  assign result_0_5_0 = $signed(result_0_5[64:1]);

  assign result_0_5_1 = result_0_5[0:0];

  assign result_1_0 = result_1[12:12];

  assign result_1_1 = result_1[11:11];

  assign result_1_2 = result_1[10:10];

  assign result_1_3 = result_1[9:9];

  assign result_1_4 = result_1[8:8];

  assign result_1_5 = result_1[7:0];

  assign result_1_5_0 = result_1_5[7:7];

  assign result_1_5_1 = result_1_5[6:6];

  assign result_1_5_2 = result_1_5[5:5];

  assign result_1_5_3 = result_1_5[4:4];

  assign result_1_5_4 = result_1_5[3:3];

  assign result_1_5_5 = result_1_5[2:2];

  assign result_1_5_6 = result_1_5[1:1];

  assign result_1_5_7 = result_1_5[0:0];


endmodule

