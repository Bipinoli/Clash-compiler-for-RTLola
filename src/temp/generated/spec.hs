...
llc event = bundle (toPop, outputs)
    where 
        ...
        out0Data0 = getMatchingTag <$> input0Win <*> ((.input0) <$> curTagsLevel1) <*> (pure (0))
...
outputStream1 :: HiddenClockResetEnable dom => Signal dom PacingOut1 -> Signal dom Tag -> Signal dom (Vec 11 Int) -> Signal dom Int -> Signal dom (Vec 2 (Tag, Int))
outputStream1 en tag sw0 in1_1 = result
    where
        result = register (repeat (invalidTag, 0)) (mux (getPacing <$> en) next result)
        next = (<<+) <$> result <*> nextValWithTag
        nextValWithTag = bundle (tag, nextVal)
        nextVal = (merge0 <$> sw0) + in1_1
