...
-- input x: Int
-- output a := x + c.offset(by: -3).defaults(to: 0) 
-- output b := a + 1
-- output c := b + 1
...
--      | a    | a    | a    | a    | a    | a    | a    | a    | a   
-- -------------------------------------------------------------------
--      |      | b,xx | b,xx | b,xx | b,xx | b,xx | b,xx | b,xx | b,xx
-- -------------------------------------------------------------------
--      |      |      | c    | c    | c    | c    | c    | c    | c   
-- -------------------------------------------------------------------
...
llc event = bundle (toPop, outputs)
    where 
        ...
        out0Data1 = getOffset <$> out2 <*> ((.output2) <$> curTagsLevel1) <*> (pure 3) <*> out0Data1Dflt
        ...
        out3Data0 = getLatestValue <$> out0 <*> out3Data0Dflt
...
outputStream2 :: HiddenClockResetEnable dom => Signal dom PacingOut2 -> Signal dom Tag -> Signal dom Int -> Signal dom (Vec 3 (Tag, Int))
...
outputStream3 en tag out0_ = result
    where
        result = register (repeat (invalidTag, 0)) (mux (getPacing <$> en) next result)
        next = (<<+) <$> result <*> nextValWithTag
        nextValWithTag = bundle (tag, nextVal)
