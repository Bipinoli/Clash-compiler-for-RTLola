...
llc event = bundle (toPop, outputs)
    where 
        ...
        curTags = Tags <$> tIn0 <*> tOut0 <*> tOut1 <*> tOut2 <*> tSw0
        ...
        out1Data1 = getMatchingTag <$> out0 <*> ((.output0) <$> curTagsLevel1) <*> (pure (0))
        ...
        sw0Data = getMatchingTag <$> out1 <*> ((.output1) <$> curTagsLevel2) <*> (pure 0)
...
outputStream2 :: HiddenClockResetEnable dom => Signal dom PacingOut2 -> Signal dom Tag -> Signal dom (Vec 101 Int) -> Signal dom (Tag, Int)
outputStream2 en tag sw0 = result
    where
