...
-- input x: Int
-- output a := x.offset(by: -1).defaults(to: 10)
-- output b := x.offset(by: -1).defaults(to: 100)
-- output c @1Hz := x.hold().defaults(to:6)
-- 
...
-- a,x,b | a,x,b | a,x,b | a,x,b | a,x,b | a,x,b | a,x,b | a,x,b | a,x,b | a,x,b
-- -----------------------------------------------------------------------------
--       | c     | c     | c     | c     | c     | c     | c     | c     | c    
-- -----------------------------------------------------------------------------
...
llc event = bundle (toPop, outputs)
    where 
        ...
        output2Aktv = delayFor d3 False (getPacing <$> pOut2)
        ...
        out0Data0 = getOffsetFromNonVec <$> input0Win <*> tIn0 <*> (pure 1) <*> out0Data0Dflt
        out0Data0Dflt = pure (10)

        -- Evaluation of output 1: level 0
        out1 = outputStream1 enOut1 tOut1 out1Data0 
        out1Data0 = getOffsetFromNonVec <$> input0Win <*> tIn0 <*> (pure 1) <*> out1Data0Dflt
        out1Data0Dflt = pure (100)

        -- Evaluation of output 2: level 1
        out2 = outputStream2 enOut2 ((.output2) <$> curTagsLevel1) out2Data0 
        out2Data0 = getLatestValueFromNonVec <$> input0Win <*> out2Data0Dflt
        out2Data0Dflt = pure (6)

        -- Outputing all results: level 2
        output0 = ValidInt <$> output0Data <*> output0Aktv
        output0Data = getMatchingTag <$> out0 <*> ((.output0) <$> curTagsLevel2) <*> (pure 0)
        output1 = ValidInt <$> output1Data <*> output1Aktv
        output1Data = getMatchingTag <$> out1 <*> ((.output1) <$> curTagsLevel2) <*> (pure 0)
...
outputStream1 en tag in0_ = result
    where
        result = register (repeat (invalidTag, 0)) (mux (getPacing <$> en) next result)
        next = (<<+) <$> result <*> nextValWithTag
        nextValWithTag = bundle (tag, nextVal)
...
outputStream2 en tag in0_ = result
    where
