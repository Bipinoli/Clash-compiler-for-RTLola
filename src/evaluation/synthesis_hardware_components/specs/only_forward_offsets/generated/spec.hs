...
-- a,b,c,x | a,b,c,x | a,b,c,x | a,b,c,x | a,b,c,x | a,b,c,x | a,b,c,x | a,b,c,x | a,b,c,x | a,b,c,x
-- -------------------------------------------------------------------------------------------------
--         | d       | d       | d       | d       | d       | d       | d       | d       | d      
-- -------------------------------------------------------------------------------------------------
...
llc event = bundle (toPop, outputs)
    where 
        ...
        output3Aktv = delayFor d3 False (getPacing <$> pOut3)
        ...
        out2Data0 = getOffset         
            ...
            <*> (pure 1) 
            <*> out2Data0Dflt
        out2Data0Dflt = getOffset <$> out1 <*> ((.output1) <$> curTagsLevel0) <*> (pure (1)) <*> (pure (0))
...
outputStream1 :: HiddenClockResetEnable dom 
    ...
    -> Signal dom Int 
    -> Signal dom (Vec 2 (Tag, Int))
outputStream1 en tag out0_0 in0_1 = result
    where
        result = register (repeat (invalidTag, 0)) 
                (mux (getPacing <$> en) next result)
        next = (<<+) <$> result <*> nextValWithTag
        nextValWithTag = bundle (tag, nextVal)
        nextVal = (out0_0 + in0_1)
...
outputStream2 :: HiddenClockResetEnable dom 
    ...
    -> Signal dom Int 
    -> Signal dom (Vec 2 (Tag, Int))
outputStream2 en tag out0_ = result
    where
        result = register (repeat (invalidTag, 0)) 
                (mux (getPacing <$> en) next result)
        next = (<<+) <$> result <*> nextValWithTag
