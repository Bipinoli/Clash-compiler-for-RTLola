...
evaluator :: HiddenClockResetEnable dom => Signal dom ((Int, Bool, Int), Bool) -> Signal dom Int -> Signal dom (Int, (Int, Bool))
evaluator controls stage = bundle (stage, resultA)
    where 
        ...
        poptime
        winXForA = windowXForA (bundle (x, newX, waitedX))
        ...
        (dataIn, enA) = unbundle controls
        (x, newX, waitedX) = unbundle dataIn
