...
module SlidingWindowNormalFreqTypeChecking where
...
newtype NewX = NewX Bool deriving (Generic, NFDataX)
...
fromNewX :: NewX -> Bool
fromNewX (NewX x) = x
toPacingA :: Bool -> PacingA
toPacingA x = PacingA x
fromPacingA :: PacingA -> Bool
fromPacingA (PacingA x) = x
toPacingB :: Bool -> PacingB
toPacingB x = PacingB x
fromPacingB :: PacingB -> Bool
fromPacingB (PacingB x) = x

---------------------------------------------------------------

type QData = (DataX, NewX)
qNullData = (toDataX 0, toNewX False) :: QData
...
pacer :: HiddenClockResetEnable dom => Signal dom Bool -> Signal dom QOutput -> Signal dom (Int, (Bool, Bool), (DataX, NewX, Int), (PacingA, PacingB), Bool)
pacer en inputBuffer = bundle (debugSignals, status, sustainedInput, enables, slides)
    where 
        ...
        enables = bundle (toPacingA <$> enA, toPacingB <$> enB)
        slides = slideB

        toPop = mux (en .&&. (stage .==. (pure 0))) (pure True) (pure False)
        pacerStable = mux (en .&&. (stage .==. (pure 1))) (pure True) (pure False)

        sustainedInput = register defaultInput (mux (en .&&. stage .==. (pure 1)) (bundle (x, toNewX <$> ((fromNewX <$> newX) .&&. qPopValid), waitedX)) sustainedInput)
        defaultInput = (toDataX 0, toNewX False, 0)
        ...
        newEnA = qPopValid .&&. (fromNewX <$> newX)
