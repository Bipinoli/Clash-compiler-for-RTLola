...
llc event = bundle (bundle (toPop, outputs), debugSignals)
    where 
        ...
        out1Tag = delay invalidTag (delay invalidTag tag)
        sw0Tag = delay invalidTag (delay invalidTag (delay invalidTag tag))
        out2Tag = delay invalidTag (delay invalidTag (delay invalidTag (delay invalidTag tag)))
        ...
        outputPhaseTag = delay invalidTag (delay invalidTag (delay invalidTag (delay invalidTag (delay invalidTag tag))))
        ...
        out0Data0 = input0Win
        out0Data1 = getOffset <$> out1 <*> out0Tag <*> (pure 3) <*> (pure 2)

        out1 = outputStream1 enOut1 out1Data0 out1Data1 
        out1Data0 = getMatchingTag <$> input1Win <*> out1Tag <*> (pure 0)
        out1Data1 = getMatchingTag <$> out0 <*> out1Tag <*> (pure 0)

        out2 = outputStream2 enOut2 out2Data0 out2Data1 
        out2Data0 = getMatchingTag <$> out0 <*> out2Tag <*> (pure 1)
        out2Data1 = sw0
